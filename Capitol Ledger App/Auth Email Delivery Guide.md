...
Capitol Ledger can create real account sessions, verification tokens, and password-reset tokens. This guide covers the outside email provider step needed to send verification and reset links to users.
...
AUTH_EMAIL_FROM="Capitol Ledger <accounts@example.com>"
...
When `AUTH_EMAIL_DELIVERY=webhook`, Capitol Ledger sends a `POST` request to `AUTH_EMAIL_WEBHOOK_URL` with:

```json
{
  "kind": "verify_email",
  "to": "user@example.com",
  "from": "Capitol Ledger <accounts@example.com>",
  "subject": "Verify your Capitol Ledger account",
...
3. Create a small webhook bridge or serverless function that accepts Capitol Ledger payloads.
