...
> Status note (September 10, 2026): Tyler clarified that the existing logo artwork stays. Commit `6ce9d5b` locally removes the hardcoded `CE` suffix from the shared wordmark cards and adds a direct regression guard; focused checks and mobile-browser QA pass. Production remains at `e82d7ea` and still shows `CE` until an exact web release is separately approved. The daily-editions migration is already applied. Use the [latest EOD](../docs/eod-handoff-2026-09-10.md) and [current timeline/task ledger](../docs/project-timeline.md) for all carryovers. The September 4–10 gap has no tracked product-work evidence, so the plan is behind its first internal checkpoint and the October 30 forecast is low-confidence/materially at risk. TestFlight is not uploaded; privacy, audit, signing, device/subscription QA and distribution/review gates remain open. The dated sections below are historical.
