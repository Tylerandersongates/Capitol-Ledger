...
> Current handoff note (September 12, 2026 EOD): exact non-production source candidate `92b61b970423ce33dc32f6a2f0a9b72a6d3e2e86` remains isolated and unmerged on `codex/sept12-privacy-neon-candidate`. Documentation checkpoint `3b78e02` is synchronized in PR #8; [exact-head CI #315](https://github.com/Tylerandersongates/Capitol-Ledger/actions/runs/34720190325), the matching Ready Vercel Preview, and targeted `/privacy/request` smoke pass. The Preview publishes the verified CapitolWonk privacy fallback while database-backed intake remains disabled. Resend verifies `capitolwonk.com`; four public aliases and Vercel Production/Preview sender/contact values are configured, the existing sending secret was preserved, and the unchanged production source was redeployed Ready with live `/privacy` smoke passing. Neon Phase 1 verified the exact literal-name application database, expected five pending migrations, and zero deletion-request rows; automatic child expiry and later drill phases remain open. Production remains `7ec68bc`; no candidate merge, production migration, intake activation, destructive QA, App Store change, signing action, upload, distribution, submission, or release occurred. Tomorrow's first task is the scheduled one-time read-only Neon child-expiry verification on September 13 at 12:00 PDT; do not delete manually. T03 remains gated by Apple issue #447 and T04 remains blocked pending Apple Support. Use the [September 12 EOD handoff](../docs/eod-handoff-2026-09-12.md) and [current timeline/task ledger](../docs/project-timeline.md). October 30 remains the user-set launch target and is low-confidence/materially at risk.
