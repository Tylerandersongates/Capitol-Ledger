...
> September 14 privacy-operations continuation: PR #13 merged the default-off aggregate monitor into `main` at `ffe994e87f8afbe8882f4fd17a6ecff95217f52f`; all three checks passed, Vercel deployment `HpDBVZ4krmgypJwZ56c47hBfttKG` reached Ready, and production gate-off smoke passed. The [September 14 owner policy](../docs/privacy-operations-policy-2026-09-14.md) approves the private-register, cadence, single-owner pause, identity, and retention decisions for implementation and isolated synthetic validation only. Every privacy, deletion, retention, and App Store activation remains off.

> September 14 custom-domain continuation: `capitolwonk.com` now redirects over HTTPS to canonical `www.capitolwonk.com` on the existing Vercel project. Public `/privacy`, `/privacy/request`, `/brief`, and `/support` smoke passed; privacy intake still returned its expected disabled 503 response. The scoped Porkbun change preserved mail, sender, wildcard, verification, and policy records. See the [custom-domain cutover record](../docs/custom-domain-cutover-2026-09-14.md).

> Superseding September 14, 2026 status: Production Batch A applied the exact five reviewed migrations and passed its complete postflight. PR #8 then merged the matching default-off privacy/deletion source into `main` at `dca8330e012f723ecd77f2756a865e907c0fa553`; PR #12 merged fail-closed App Store verifier containment at `4836e3e48d95d677f8b64713a7f8af62204ea631`; and PR #13 merged the default-off aggregate privacy monitor at `ffe994e87f8afbe8882f4fd17a6ecff95217f52f`. GitHub checks and the matching Vercel production deployments passed; deployment `HpDBVZ4krmgypJwZ56c47hBfttKG` for `ffe994e` is Ready and targeted gate-off smoke passed. Privacy intake, the monitor, deletion, retention, App Store verification, and Notifications V2 remain off. T04 remains frozen pending substantive Apple Support guidance. The active Apple-independent T09 work is the approved owner policy and its future isolated synthetic lifecycle harness. The dated September 13 notes below are historical where they conflict with this paragraph. Use the [superseding task-ledger reconciliation](../docs/project-timeline.md#september-14-superseding-reconciliation).
