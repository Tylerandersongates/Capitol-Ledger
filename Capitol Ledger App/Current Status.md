...
- The App Store subscription path has a direct StoreKit native shell plus a server validation endpoint for account-wide Pro and Team sync. App Store Connect already contains the recorded 38-product inventory; remaining work is product ordering/review metadata and exact-ID validation, App Store Server API credential/runtime validation, and sandbox/TestFlight purchase QA—not duplicate product creation.
...
- TestFlight is the active prep direction. The launch-facing text-tone pass covers auth/account, dashboard, settings, upgrade, feedback, alerts, brief, search empty states, member source placeholders, locked feature cards, and bill/member detail sub-tabs. The immediate path is T04 Apple signing guidance plus T09's local privacy/deletion candidate review; App Store Connect publication and sandbox/TestFlight purchase QA follow only after their named gates and approvals.
...
- The `/sign-in` screen has a demo-ready auth flow plus production-shaped auth endpoints. The current account milestone is the ordered deletion-integrity/cleanup-outbox/Team-pause migration and runtime-verification plan, alongside final auth-email delivery and end-to-end account QA; no generic or repeated database migration is authorized.
