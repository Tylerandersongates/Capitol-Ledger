...
> Current working-candidate note (September 11, 2026): the live candidate is now an **uncommitted, unfrozen working tree** on `codex/logo-refresh-sept10`, not `a6cb1da`. It adds the official Apple server dependency, canonical App Store state and hash-only Notifications V2 receipts, server reconciliation, Apple/Team fail-closed transitions, the fourth candidate-to-production migration `20260911110000_app_store_server_state`, exact default-off deletion and privacy-retention gates, and telemetry/log minimization. The earlier `a6cb1da` CI/Ready Preview/smoke and `f4f04de` dependency-audit closure remain valid September 10 history but are stale for this working candidate. The pnpm 9 lock now forces Next `15.5.25` from `postcss@8.4.31` to `8.5.25`; September 11 production/full working-tree audits both report **no known vulnerabilities**, and CI plus the strict candidate command now run both audits. Deprecated/unmaintained production transitive `jsrsasign@11.1.5`, used through Apple's official server library `3.1.0` for X.509/OCSP, has no current advisory but still requires upstream monitoring/upgrade and Tyler's explicit risk acceptance before launch. The bundle remains unapproved and needs frozen-install/audit, exact-head CI, matching Preview, build/smoke, provider, sandbox, and device evidence. See the [September 11 dependency note](../docs/dependency-security-audit-2026-09-11.md). The read-only production preflight earlier September 11 found only the three September 10 migrations pending; because the fourth migration was added afterward, a new preflight must require the exact four-item set before any action. Deletion and retention gates are implemented in source but are not approved, deployed, configured, or activated. Apple purchase/renewal/restore/refund/notification behavior and Team acknowledgement/restoration still require the [App Store sandbox QA matrix](../docs/app-store-sandbox-qa-matrix-2026-09-11.md) on one signed candidate. Production `main` remains `7ec68bc`; no migration, deployment, protected configuration, App Store Connect change, account deletion, retention sweep, provider mutation, restore, signing action, upload, distribution, or release occurred. T04 remains frozen pending Apple Support, and the public channel still has no first video or player.
...
The checks below were recorded for earlier committed candidates. They do not clear the uncommitted September 11 working candidate; repeat the applicable matrix after one commit is frozen.
...
- The `/sign-in` screen has a demo-ready auth flow plus production-shaped auth endpoints. The current account milestone is the exact four-migration order—deletion integrity, cleanup outbox, Team-pause workspace integrity, then App Store server state—plus gated runtime verification, final auth-email delivery, and end-to-end account QA; no migration is authorized.
