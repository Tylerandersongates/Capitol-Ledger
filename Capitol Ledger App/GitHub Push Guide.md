...
Push the local Capitol Ledger app folder to the GitHub repository that Vercel will deploy from.

## Why This Is Needed

Vercel can be connected to your GitHub account, but it still needs the actual Capitol Ledger code inside a GitHub repository. The local app folder is not a Git repository until `git init` is run from the project folder.
...
git add .
git status --short
git commit -m "Initial Capitol Ledger app"
