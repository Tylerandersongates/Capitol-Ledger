...
1. **Weekend, September 26–27:** no scheduled project work. Passive Production monitoring only; act only on a material incident.
2. **Monday, September 28:** confirm post-merge Production health; identify the exact branch-scoped Preview `DATABASE_URL` and `AUTH_EMAIL_FROM` cleanup targets; confirm whether the temporary Neon child auto-expired. Request exact action-time confirmation before removing any environment override. Never reveal or recreate credentials, tokens, links, or disposable-user data.
3. **By October 1:** finish the whole-app launch-gate inventory and preserve a clean pre-absence baseline with every unfinished T01–T12/deferred item, owner, dependency, and next action visible.
4. **October 7–November 13:** run the controlled-web regression, trusted-user preparation, support/monitoring/privacy-mailbox checks, civic-data freshness/source review, content decisions, rollback rehearsal, and final go/no-go packet. Keep unverified first-party privacy and App Store processing paths off.
5. **November 16–January 3:** after the controlled soft launch, prioritize wider-use defects and reliability. Complete T12's 120th Congress roster, district, bill, vote, source/sync, copy, and metadata transition before the January 2 full-launch go/no-go.

**Single safest next action:** on Monday, run a read-only post-merge health check and exact cleanup-target review from the September 25 handoff. Use the [active backward plan](../docs/project-timeline.md#september-25-trust-repair-completion-and-two-stage-launch-rebaseline); the older instructions below are historical.
