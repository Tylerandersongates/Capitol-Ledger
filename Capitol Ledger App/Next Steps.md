...
Current September 13 handoff: exact non-production source candidate `92b61b970423ce33dc32f6a2f0a9b72a6d3e2e86` remains isolated and unmerged on `codex/sept12-privacy-neon-candidate`. Documentation checkpoint `64bc914` is pushed in PR #8; exact-head CI #317 and the matching Ready Preview pass. Controlled-domain email and Vercel sender/contact configuration are complete, the unchanged production source was redeployed Ready, and production remains `7ec68bc`. Neon Phase 1 passes with zero deletion-request rows and exactly five pending candidate migrations. **Neon cleanup is complete for the approved scope:** the temporary child and compute expired automatically; the final read-only check proved production application reads target literal `Capitol%20Ledger`, while spaced-name `Capitol Ledger` had zero public tables and no application dependency; Tyler then separately confirmed permanent deletion of only that empty database. Neon now retains `neondb` and `Capitol%20Ledger`, and the post-delete production read smoke passed. Keep PR #8 unmerged and intake disabled, preserve the T03 Apple-library fix watch and T04 signing freeze, and continue only already-scoped runtime/retention/operations evidence. Read the [September 12 EOD handoff](../docs/eod-handoff-2026-09-12.md) and [current timeline](../docs/project-timeline.md) before acting. October 30 remains the user-set launch target and is low-confidence/materially at risk; no further database deletion, migration, candidate deployment, activation, test message, destructive QA, App Store change, signing, upload, distribution, submission, or release is authorized by this handoff.
