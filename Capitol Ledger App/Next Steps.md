...
Current September 11 source candidate: exact committed branch candidate `3dbba3a260b10924dff254deed7f65a5e392c239` adds the official Apple server dependency, canonical App Store state/Notifications V2 reconciliation, Apple/Team fail-closed handling, the fourth candidate-to-production migration `20260911110000_app_store_server_state`, exact default-off deletion and privacy-retention gates, telemetry/log minimization, and blank-account demo isolation. Frozen dependency/source evidence, the repeated read-only four-migration preflight, [exact-head CI `34668039916`](https://github.com/Tylerandersongates/Capitol-Ledger/actions/runs/34668039916), matching Ready Vercel deployment `6K9Xd24c4Pg4Nz1pRLorNzo6MxGb`, and targeted anonymous Preview smoke pass. Provider, sandbox, and device evidence remain open. The gates are committed and source-verified but are not approved, deployed, configured, or activated. Apple and Team flows still require the [App Store sandbox QA matrix](../docs/app-store-sandbox-qa-matrix-2026-09-11.md) on one signed candidate. Production `main` remains `7ec68bc`; T04 remains blocked pending Apple; no production, provider, App Store Connect, signing, device, upload, distribution, or release action occurred.

The [September 11 backup/PITR and provider-retention checklist](../docs/backup-pitr-provider-retention-evidence-2026-09-11.md) is prepared, not closed. The pnpm 9 lock now forces Next's PostCSS to `8.5.25`, and the September 11 frozen production/full audits report **no known vulnerabilities**; the successful exact-head CI reruns both audits. Production transitive `jsrsasign@11.1.5` is deprecated/unmaintained and used by Apple's official server library for X.509/OCSP; it has no current advisory, but upstream monitoring/upgrade and Tyler's explicit risk acceptance remain launch gates. See the [September 11 dependency note](../docs/dependency-security-audit-2026-09-11.md). The next safe work is to obtain read-only evidence for the exact Neon restore window/backups, Vercel retention/drains, Sentry products/scrub/retention, and enabled email/webhook boundaries. Tyler still must approve the constrained restore floor and the exact default-off deployment/activation sequence. Source-level URL/token scrubbing and retention switches do not prove Vercel/Sentry/drain behavior or authorize a sweep. Resolve the Sentry one-feedback-removal contradiction before public privacy publication.
...
- T01 is complete at production source `7ec68bc`, and T02 is complete for new Sentry events under its recorded scope. T03 remains historically complete for `f4f04de`; the September 11 frozen graph is advisory-clean, but deprecated/unmaintained `jsrsasign@11.1.5` remains a production maintenance-risk gate and needs upstream monitoring/upgrade plus Tyler's explicit acceptance if it remains at launch. Candidate `3dbba3a` has frozen dependency/source/preflight evidence, passing exact-head CI, a matching Ready Preview, and targeted smoke. T04's read-only signing-state reconciliation is complete, and T04 is blocked pending Apple Developer Support's reply; preserve the signing freeze and present one exact action for Tyler's approval after the reply is inspected. Corrected-wordmark listing screenshot recapture remains under T09.
