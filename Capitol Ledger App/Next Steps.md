...
Superseding September 14 next step: keep every privacy and App Store processing gate off, preserve the T04 signing/device freeze while Apple Support is pending, and continue T09 locally. PR #19 merged the strict privacy operator parser/dispatcher core into `main` at `b92ad1dde374638f0fd584a2645d4437bd76853c`; all three GitHub checks passed, exact-source Vercel production deployment `BejdtoqtPvpa9LtbKcz5Y6VSro73` reached Ready with `www.capitolwonk.com`, the apex redirects to canonical `www`, `/privacy` returns 200, and the privacy API remains disabled with `503` and `no-store`. The active local candidate now adds only the bounded standard-input transport for that core and intentionally binds no adapter, so an allowed local command still fails closed without touching a database, mailbox, export store, provider, filesystem, or network. It adds no principal, credential, production binding, route, scheduler, migration, configuration change, or activation. Next, finish exact-source checks and review this candidate; then present its PR as a separate external action. Do not connect to production, apply the operations migration, schedule a job, read or write production data, send mail, call a provider, delete an artifact, restore data, switch traffic, run retention, change Vercel configuration, or enable a gate. Production Batch A and PRs #8, #12, #13, #14, #15, #16, #17, #18, and #19 are complete; the dated September 13 instructions below are historical where they say those actions remain pending. Use the [September 14 task-ledger reconciliation](../docs/project-timeline.md#september-14-superseding-reconciliation).
