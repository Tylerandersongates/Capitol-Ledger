...
September 15 priority correction — current next action: Tyler completes the protected production `neondb_owner` password reset directly in Neon; do not reuse, display, copy, or record the exposed value, and clear any clipboard copy of the replacement. The sanitized [containment record](../docs/neon-owner-credential-containment-2026-09-15.md) is the durable boundary. The principal-ACL inventory never ran and must not be retried. Vercel project Environment Variables showed no `DATABASE_URL`, linked Shared Environment Variables were absent, and no connected Vercel database resource was present; that narrow inspection does not rule out an unknown external client. Once Tyler reports the reset complete, record only completion and close containment. The first-party privacy operator/ACL/migration/role sequence is conditional post-launch work unless Tyler explicitly restores it to launch scope. Use the configured dedicated privacy mailbox for launch and keep first-party intake, operations, monitoring, retention, deletion, the adapter, and all App Store processing gates off.

Next Apple-independent launch work after containment: finish the source-only App Privacy, listing-metadata, and corrected-wordmark screenshot/capture reconciliation; keep watching for an official fix to the Apple verifier dependency; finish read-only provider/retention evidence; and make the T08 Daily Brief/video launch-scope decision. The real release critical path remains T03/T04, followed by signed-candidate T05–T07, then T10/TestFlight, review, and T11 launch. Preserve the T04 signing/device freeze until substantive Apple Support guidance is documented. Routine source-only preparation, validation, commits, pushes, and low-risk documentation merges proceed under the [standing EOD rules](../docs/eod-handoff-template.md); protected settings, production writes, behavior-changing deployments, Apple account/signing actions, uploads, distribution, submission, and release still require exact approval.
