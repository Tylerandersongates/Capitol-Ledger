...
Superseding September 14 next step: keep every privacy and App Store processing gate off, preserve the T04 signing/device freeze while Apple Support is pending, and continue T09 locally. PR #21 merged the explicit-dependency privacy operator service adapter into `main` at `f83be73c4fdd34ffb9f8cef2faa68837d3985528`; all three GitHub checks passed, exact-source Vercel production deployment `7HfnCVfeNTjHYvHmiyvkkWNmAyuU` reached Ready with `www.capitolwonk.com`, the apex redirects to canonical `www`, `/privacy` returns 200, and the privacy API remains disabled with `503` and `no-store`. The adapter remains disconnected from the stdin shell and all application routes. The active local candidate now records only a function-mediated least-privilege database-access contract for a future dedicated non-owner principal and inventories the existing eight fixed SQL documents across `PrivacyRequest` and `PrivacyRequestOperation`. It contains no function SQL, role, credential, grant execution, connection bootstrap, production binding, route, scheduler, provider action, migration, configuration change, or activation; direct table DML remains unapproved because PostgreSQL `DELETE` is not column-scoped. Next, finish exact-source checks and review this candidate; then present its PR as a separate external action. Do not connect to production, apply the operations migration, create a principal or credential, execute a grant, schedule a job, read or write production data, send mail, call a provider, delete an artifact, restore data, switch traffic, run retention, change Vercel configuration, or enable a gate. Production Batch A and PRs #8, #12, #13, #14, #15, #16, #17, #18, #19, #20, and #21 are complete; the dated September 13 instructions below are historical where they say those actions remain pending. Use the [September 14 task-ledger reconciliation](../docs/project-timeline.md#september-14-superseding-reconciliation).
