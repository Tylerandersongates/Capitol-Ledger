...
Superseding September 14 next step: keep every privacy and App Store processing gate off, preserve the T04 signing/device freeze while Apple Support is pending, and continue T09 locally. PR #17 merged the pure operator-boundary and pause/return checkpoint into `main` at `0f3861b448471bfc9ad8ff012ccb94e152ae31e4`; all three GitHub checks passed, Vercel production deployment `DtpFr58g2BHmgBAHhuhmXU6R5gF2` reached Ready at that exact source with `www.capitolwonk.com`, the apex redirects to canonical `www`, `/privacy` returns 200, and the privacy API remains disabled with `503` and `no-store`. The active local candidate now adds only pure, sanitized mailbox/export deletion-evidence and constrained restore-floor verifiers plus a machine-readable contract and fixtures. It implements no provider/storage deletion, database/mailbox read, restore, traffic switch, route, credential, scheduler, migration, configuration change, or activation. Next, finish exact-source checks and review this candidate; then present its PR as a separate external action. Do not connect to production, apply the operations migration, schedule a job, read or write production data, send mail, call a provider, delete an artifact, restore data, switch traffic, run retention, change Vercel configuration, or enable a gate. Production Batch A and PRs #8, #12, #13, #14, #15, #16, and #17 are complete; the dated September 13 instructions below are historical where they say those actions remain pending. Use the [September 14 task-ledger reconciliation](../docs/project-timeline.md#september-14-superseding-reconciliation).
