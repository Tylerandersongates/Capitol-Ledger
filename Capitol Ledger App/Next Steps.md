...
Current September 13 handoff: exact non-production source candidate `92b61b970423ce33dc32f6a2f0a9b72a6d3e2e86` remains isolated and unmerged on `codex/sept12-privacy-neon-candidate`. Documentation checkpoint `3b78e02` is pushed in PR #8; [exact-head CI #315](https://github.com/Tylerandersongates/Capitol-Ledger/actions/runs/34720190325), the matching Ready Preview, and targeted privacy-fallback gate-off smoke pass. Controlled-domain email and Vercel sender/contact configuration are complete, the unchanged production source was redeployed Ready, and production remains `7ec68bc`. Neon Phase 1 passes with zero deletion-request rows and exactly five pending candidate migrations. **The scheduled read-only cleanup check passed:** only protected `production` remains, so the temporary child and compute expired automatically; the production database list is unchanged. Next, run one final read-only dependency/target check before presenting the empty spaced-name `Capitol Ledger` database for separate permanent-delete confirmation. Keep PR #8 unmerged and intake disabled, preserve the T03 Apple-library fix watch and T04 signing freeze, and pull forward only already-scoped evidence/operations preparation. Read the [September 12 EOD handoff](../docs/eod-handoff-2026-09-12.md) and [current timeline](../docs/project-timeline.md) before acting. October 30 remains the user-set launch target and is low-confidence/materially at risk; no database deletion, migration, candidate deployment, activation, test message, destructive QA, App Store change, signing, upload, distribution, submission, or release is authorized by this handoff.
...
The [September 11 backup/PITR and provider-retention checklist](../docs/backup-pitr-provider-retention-evidence-2026-09-11.md) has current settings evidence plus verified Neon Phase 1 and automatic cleanup, but is not closed. The pnpm 9 lock forces Next's PostCSS to `8.5.25`, and the frozen production/full audits report **no known vulnerabilities**; exact-head CI #315 reruns both audits. Production transitive `jsrsasign@11.1.5` is deprecated/unmaintained and used by Apple's official server library for X.509/OCSP; Apple issue #447 affects the enabled OCSP-freshness path, so an official fix or separately approved reviewed fallback plus revalidation is a release gate. See the [September 12 risk packet](../docs/jsrsasign-risk-decision-2026-09-12.md) and [September 11 dependency note](../docs/dependency-security-audit-2026-09-11.md). The next safe work is the final read-only database dependency/target check, followed by the remaining exact runtime/retention/operations evidence that does not require mutation. Tyler still must separately approve every permanent database deletion, later restore, migration, deployment, activation, delivery exercise, destructive QA, and App Store action.
...
- T01 is complete at production source `7ec68bc`, and T02 is complete for new Sentry events under its recorded scope. The frozen graph is advisory-clean, but T03 is high/open because Apple issue #447 affects the enabled OCSP path through end-of-life `jsrsasign@11.1.5`. Current source candidate `92b61b9` plus documentation checkpoint `3b78e02` have passing exact-head CI #315, a matching Ready Preview, and privacy-fallback gate-off smoke. T04 is blocked pending Apple Developer Support; preserve the signing freeze and present one exact action for Tyler's approval only after a reply is inspected. T05–T11 and every approval/deferred-scope item are carried in the current timeline and September 12 EOD handoff. Corrected-wordmark listing screenshot recapture remains under T09.
