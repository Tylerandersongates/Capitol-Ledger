...
Current September 11 working candidate: an **uncommitted, unfrozen working tree** adds the official Apple server dependency, canonical App Store state/Notifications V2 reconciliation, Apple/Team fail-closed handling, the fourth candidate-to-production migration `20260911110000_app_store_server_state`, exact default-off deletion and privacy-retention gates, and telemetry/log minimization. The expected pending migration order is now the three September 10 migrations followed by the App Store state migration, subject to a fresh read-only status check. The gates are implemented in source but are not candidate-verified, approved, deployed, configured, or activated. Apple and Team flows still require the [App Store sandbox QA matrix](../docs/app-store-sandbox-qa-matrix-2026-09-11.md) on one signed candidate. Production `main` remains `7ec68bc`; T04 remains blocked pending Apple; no production, provider, App Store Connect, signing, device, upload, distribution, or release action occurred.

The [September 11 backup/PITR and provider-retention checklist](../docs/backup-pitr-provider-retention-evidence-2026-09-11.md) is prepared, not closed. The pnpm 9 lock now forces Next's PostCSS to `8.5.25`, and September 11 production/full working-tree audits both report **no known vulnerabilities**. CI and the strict candidate command now run both audits. The candidate remains uncommitted/unfrozen; repeat both audits after a frozen install. Production transitive `jsrsasign@11.1.5` is deprecated/unmaintained and used by Apple's official server library for X.509/OCSP; it has no current advisory, but upstream monitoring/upgrade and Tyler's explicit risk acceptance remain launch gates. See the [September 11 dependency note](../docs/dependency-security-audit-2026-09-11.md). The next safe work is to freeze and verify one candidate, then obtain read-only evidence for the exact Neon restore window/backups, Vercel retention/drains, Sentry products/scrub/retention, and enabled email/webhook boundaries. Tyler still must approve the constrained restore floor and the exact default-off deployment/activation sequence. Source-level URL/token scrubbing and retention switches do not prove Vercel/Sentry/drain behavior or authorize a sweep. Resolve the Sentry one-feedback-removal contradiction before public privacy publication.
...
- T01 is complete at production source `7ec68bc`, and T02 is complete for new Sentry events under its recorded scope. T03 remains historically complete for `f4f04de`. For the September 11 working graph, Next's PostCSS is forced to `8.5.25` and production/full audits now report no known vulnerabilities; frozen-candidate evidence is still required. Deprecated/unmaintained `jsrsasign@11.1.5` remains a production maintenance-risk gate despite having no current advisory, so upstream monitoring/upgrade and Tyler's explicit acceptance are required before launch. T04's read-only signing-state reconciliation is complete, and T04 is blocked pending Apple Developer Support's reply; preserve the signing freeze and present one exact action for Tyler's approval after the reply is inspected. Corrected-wordmark listing screenshot recapture remains under T09.
