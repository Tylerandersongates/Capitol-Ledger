...
Superseding September 14 next step: keep every privacy and App Store processing gate off, preserve the T04 signing/device freeze while Apple Support is pending, and continue T09 locally. PR #15 merged the default-off privacy operator lifecycle into `main` at `8c31faf29897b0f0ea1552ee8e0c20429297223e`; all three GitHub checks passed, Vercel production deployment `6RMhX6i4cT6Z4TmNVNXvTbaan6hz` reached Ready, the apex still redirects to canonical `www`, `/privacy` and `/privacy/request` passed, and the API returned its expected disabled `503` with `no-store`. The source migration remains unapplied and privacy operations, intake, monitoring, retention, deletion, and App Store processing remain off. The next Apple-independent task is a local-only least-privileged operator-runner/access-boundary plan plus the exact single-owner Vercel pause/return runbook and static/synthetic checks. Do not bind protected credentials, connect the runner to production, apply the migration, schedule a job, read or write production data, send mail, call a provider, run retention, or enable a gate. Production Batch A and PRs #8, #12, #13, #14, and #15 are complete; the dated September 13 instructions below are historical where they say those actions remain pending. Use the [September 14 task-ledger reconciliation](../docs/project-timeline.md#september-14-superseding-reconciliation).
