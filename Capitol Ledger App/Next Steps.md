...
Superseding September 14 next step: keep every privacy and App Store processing gate off, preserve the T04 signing/device freeze while Apple Support is pending, and continue T09 locally. PR #16 merged the PR #15 deployment/task-ledger reconciliation into `main` at `87e7d1f96f538d4d9509194ee150a34779f97a0a`; all three GitHub checks passed, Vercel production deployment `HJLAQL66uh57DN2P4bv31v7oF4mZ` reached Ready at that exact source, the apex still redirects to canonical `www`, and the privacy API returned its expected disabled `503` with `no-store`. The active local candidate now adds only a pure fail-closed operator guard, a machine-readable least-privilege contract, the exact single-owner Vercel pause/return runbook, and static/synthetic checks. It contains no executable runner, application route, protected credential, production binding, scheduler, or external action. Next, finish exact-source checks and review this candidate; then present its PR as a separate external action. Do not connect to production, apply the operations migration, schedule a job, read or write production data, send mail, call a provider, run retention, change Vercel configuration, or enable a gate. Production Batch A and PRs #8, #12, #13, #14, #15, and #16 are complete; the dated September 13 instructions below are historical where they say those actions remain pending. Use the [September 14 task-ledger reconciliation](../docs/project-timeline.md#september-14-superseding-reconciliation).
