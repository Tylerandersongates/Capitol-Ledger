...
September 15 current next step: Keep routine source-only work moving under the [standing EOD rules](../docs/eod-handoff-template.md), but stop the T09 production operator sequence at the failed ambient-privilege baseline. The protected production read found the operations-table count and shared `PUBLIC` database privileges out of contract; separate Neon UI evidence confirmed `PrivacyRequestOperation` is missing and its migration name is not recorded. Review the [September 15 evidence](../docs/privacy-request-operator-read-only-evidence-2026-09-15.md) and [remediation plan](../docs/privacy-request-operator-ambient-privilege-remediation-review-2026-09-15.md). The next protected proposal is the digest-pinned, catalog-only [principal-ACL inventory](../docs/privacy-request-operator-principal-acl-inventory-2026-09-15.sql) through a direct client that validates the literal database and handles a credential without disclosure; it needs its own exact approval. Identifying Vercel's runtime database username from a private setting is a different approval. Only after those dependencies are known may the operations migration and shared-ACL repair be independently reviewed for separate production-write approvals. Do not repeat Production Batch A or the failed preflight. Keep the adapter disconnected, all processing gates off, and T04 signing/device frozen. The September 14 and September 13 instructions below are historical where superseded.
