...
Superseding September 14 next step: keep every privacy and App Store processing gate off, preserve the T04 signing/device freeze while Apple Support is pending, and continue T09 locally. PR #18 merged the pure artifact-deletion evidence and constrained restore-floor verifiers into `main` at `aca87f50860c53c542c32e5928e19f28e582a76b`; all three GitHub checks passed, Vercel production deployment `3MF99WfbPcGxL7iMSWoDmKhgj9eC` reached Ready at that exact source with `www.capitolwonk.com`, the apex redirects to canonical `www`, `/privacy` returns 200, and the privacy API remains disabled with `503` and `no-store`. The active local candidate now adds only the bounded one-document parser and injected dispatcher core for the future privacy operator command, a machine-readable contract, a least-privilege design floor, and synthetic/static checks. It adds no executable entrypoint, stdin reader, database adapter, principal, credential, provider/storage action, production binding, route, scheduler, migration, configuration change, or activation. Next, finish exact-source checks and review this candidate; then present its PR as a separate external action. Do not connect to production, apply the operations migration, schedule a job, read or write production data, send mail, call a provider, delete an artifact, restore data, switch traffic, run retention, change Vercel configuration, or enable a gate. Production Batch A and PRs #8, #12, #13, #14, #15, #16, #17, and #18 are complete; the dated September 13 instructions below are historical where they say those actions remain pending. Use the [September 14 task-ledger reconciliation](../docs/project-timeline.md#september-14-superseding-reconciliation).
