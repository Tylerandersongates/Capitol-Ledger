...
Current September 13 handoff: exact non-production source candidate `92b61b970423ce33dc32f6a2f0a9b72a6d3e2e86` remains isolated and unmerged on `codex/sept12-privacy-neon-candidate`. Cleanup checkpoint `0d07e10` is pushed in PR #8; exact-head CI #318 and the matching Vercel Preview pass. Controlled-domain email and Vercel sender/contact configuration are complete, the unchanged production source was redeployed Ready, and production remains `7ec68bc`. **Neon cleanup is complete for the approved scope:** the temporary child and compute expired automatically; final evidence proved production reads target literal `Capitol%20Ledger`; Tyler separately confirmed permanent deletion of only the empty spaced-name database. Neon now retains `neondb` and `Capitol%20Ledger`, and post-delete production read smoke passed. Apple Support still has not replied and Tyler plans to call September 14, so keep the T04 signing/device freeze intact. While waiting, first prepare the exact five-migration promotion packet and privacy-operations/single-owner contingency; then continue only read-only provider/retention evidence, local screenshot preparation, App Privacy reconciliation, Daily Brief planning, or the official Apple-library fix watch. Read the [September 13 handoff](../docs/eod-handoff-2026-09-13.md) and [current timeline](../docs/project-timeline.md) before acting. Keep PR #8 unmerged and privacy intake disabled. October 30 remains the user-set launch target and is low-confidence/materially at risk; no further database deletion, migration, candidate deployment, activation, test message, destructive QA, App Store change, signing, upload, distribution, submission, or release is authorized by this handoff.
...
6. Confirm the privacy policy, support URL, data retention, and account-deletion evidence for Apple review. The earlier read-only production preflight found `20260718154000_account_deletion_requests` applied and the three September 10 migrations pending. The working candidate then added `20260911110000_app_store_server_state`, so a fresh preflight must require the exact four-item order. Follow the separately gated approval packet for future deployment, live schema/FK/Apple-ownership inspection, protected cleanup-task scheduling/monitoring/retries/completed-job erasure, disposable-account plus Team-member/legacy-Stripe cleanup QA, physical-device local/session clearing, backup/PITR tombstone policy, and Apple/Stripe/Sentry/email/official-message retention/removal procedures.
...
- T01 is complete at production source `7ec68bc`, and T02 is complete for new Sentry events under its recorded scope. The frozen graph is advisory-clean, but T03 is high/open because Apple issue #447 affects the enabled OCSP path through end-of-life `jsrsasign@11.1.5`. Current source candidate `92b61b9` plus cleanup checkpoint `0d07e10` have passing exact-head CI #318, a matching Vercel Preview, and privacy-fallback gate-off smoke. T04 is blocked pending Apple Developer Support; preserve the signing freeze and present one exact action for Tyler's approval only after a reply or call guidance is inspected. T05–T11 and every approval/deferred-scope item are carried in the current timeline and September 13 handoff. Corrected-wordmark listing screenshot recapture remains under T09.
