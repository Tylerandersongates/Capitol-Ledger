...
Superseding September 14 next step: keep every privacy and App Store processing gate off, preserve the T04 signing/device freeze while Apple Support is pending, and continue T09 locally. PR #20 merged the fail-closed privacy operator stdin shell into `main` at `72e9a8cea004d351ca12d44efcf15898d0b047ed`; all three GitHub checks passed, exact-source Vercel production deployment `FgFUuXoSrbHWVFQM82tV81pKqcUh` reached Ready with `www.capitolwonk.com`, the apex redirects to canonical `www`, `/privacy` returns 200, and the privacy API remains disabled with `503` and `no-store`. The active local candidate now adds only an adapter factory that composes the existing lifecycle and aggregate-monitor services when its caller explicitly injects a database, environment, and clock. The stdin shell remains intentionally unbound, and the candidate adds no production principal, credential, connection bootstrap, production binding, route, scheduler, provider action, migration, configuration change, or activation. Next, finish exact-source checks and review this candidate; then present its PR as a separate external action. Do not connect to production, apply the operations migration, schedule a job, read or write production data, send mail, call a provider, delete an artifact, restore data, switch traffic, run retention, change Vercel configuration, or enable a gate. Production Batch A and PRs #8, #12, #13, #14, #15, #16, #17, #18, #19, and #20 are complete; the dated September 13 instructions below are historical where they say those actions remain pending. Use the [September 14 task-ledger reconciliation](../docs/project-timeline.md#september-14-superseding-reconciliation).
