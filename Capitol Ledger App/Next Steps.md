...
Superseding September 14 next step: keep every privacy and App Store processing gate off, preserve the T04 signing/device freeze while Apple Support is pending, and advance T09 locally. The first implementation slice is the exact-opt-in aggregate privacy queue monitor on `codex/privacy-operations-monitor`; it exposes only `new`/`reviewing` counts, oldest age bands, and resolved counts by closed category, with no case/user/payload identifiers. It is not deployed, scheduled, configured, or run against production. Next, Tyler must approve or revise the private-register boundary, review cadence, single-owner absence rule, identity escalation, and retention worksheet before an isolated synthetic lifecycle exercise. Production Batch A, PR #8, and PR #12 are complete; the dated September 13 instructions below are historical where they say those actions remain pending. Use the [September 14 task-ledger reconciliation](../docs/project-timeline.md#september-14-superseding-reconciliation).
