...
Weekly Brief delivery is now ready for a production provider, but the app does not need to choose that provider yet. CapitolWonk can send a provider-agnostic webhook payload to an email or push bridge, while continuing to record delivery history in the app.
...
WEEKLY_BRIEF_FROM="CapitolWonk <briefs@example.com>"
```
...
When `WEEKLY_BRIEF_DELIVERY=webhook`, CapitolWonk sends a `POST` request to `WEEKLY_BRIEF_WEBHOOK_URL` with:

```json
{
  "kind": "weekly_brief",
  "to": "user@example.com",
  "from": "CapitolWonk <briefs@example.com>",
  "subject": "CapitolWonk Daily Civic Brief",
