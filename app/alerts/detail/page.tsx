...
export default function AlertDetailPage() {
  ...
  const districtMember = getAllMembers().find((member) => member.state === "TX") ?? getAllMembers()[0];
  ...
  const alertDetails = [
    ...
    { label: "Your Position", value: "Support", icon: <UserCircle />, tone: "text-[#43ed74]" },
    { label: "Your Rep.", value: districtMember?.fullName.replace(/^Sen\.\s+|^Rep\.\s+/, "") ?? "Representative", icon: <UserCircle /> }
  ];
  ...
  return (
    ...
    >
            ...
            <header className="relative mt-10 flex items-center justify-center">
              <Link href="/alerts" className={`absolute left-0 ${mobileIconButtonClass}`} aria-label="Back to alerts">
            ...
            <main className="mt-7 pb-5">
              ...
              <div className="mt-4 space-y-3">
                <GamificationEventLink href="/impact" event="contact-representative" targetId="vote-reminder-action" className="flex h-12 items-center justify-center rounded-xl bg-gradient-to-r from-[#ffdf63] via-[#ffb12b] to-[#ff8a00] text-[17px] font-semibold text-[#071225] shadow-[0_0_24px_rgba(255,177,43,0.22)]">
                  Take Action
