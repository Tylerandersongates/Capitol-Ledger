...
export async function POST(request: Request) {
  ...
  Sentry.withScope((scope) => {
    ...
    scope.setTag("capitolwonk.runtime", "nodejs");
    Sentry.captureException(new Error("CapitolWonk sanitized synthetic Node.js diagnostic"));
  });

  const flushed = await Sentry.flush(5_000);

  return Response.json(
    { accepted: true, flushed, runtime: "nodejs" },
