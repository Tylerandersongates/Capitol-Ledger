...
function buildVoteDots(totals: { yes: number; no: number; notVoting: number }) {
  ...
  return Array.from({ length: 100 }).map((_, index) => {
    ...
    return {
      ...
      color: index < yesEnd ? "#39d884" : index < noEnd ? "#ff503d" : "#9ca5b1"
    };
  });
}
...
export default function VoteDetailPage({ params }: VotePageProps) {
  ...
  const voteDots = buildVoteDots(totals);
  ...
  return (
    ...
    >
      ...
      <header className="mt-10 flex items-center justify-between">
        <Link href="/search?type=votes" className={mobileIconButtonClass} aria-label="Back to votes">
      ...
      <main className="mt-7 space-y-4 pb-8">
        <MobileCard variant="dashboard" className="px-5 py-5">
          ...
          <div className="relative mx-0 mt-5 h-32 w-full" aria-hidden="true">
...
function VoteStat({ value, label, tone }: { value: string; label: string; tone: string }) {
  return (
    <div className="border-r border-white/8 last:border-r-0">
      <div className={`text-[31px] font-medium leading-none ${tone}`}>{value}</div>
      <div className="mt-2 text-[16px] text-white/67">{label}</div>
    </div>
  );
}
