...
export function VoterRegistrationCard() {
  ...
  function markComplete() {
    ...
    if (!pending) {
      setPending(true);
      setStatus("Tap again to confirm voter registration form completion.");
      return;
    }
    ...
    setComplete(true);
    setPending(false);
    setStatus("Voter registration form complete.");
  }
    ...
    <div id="voter-registration">
      <MobileCard variant="dashboard" className="px-5 py-5">
        ...
        <p className="mt-3 text-[13px] leading-relaxed text-white/58">
          Complete a Voter Registration Form to unlock Register to Vote.
        </p>
        ...
        >
          ...
          {isComplete ? "Completed" : pending ? "Confirm Completion" : "Mark Form Complete"}
        </button>
        ...
        <div className="mt-3 flex items-start gap-2 rounded-xl border border-[#79a8ff]/18 bg-[#79a8ff]/8 px-3 py-2 text-[12px] text-[#b8d2ff]">
          ...
          <span>{status || "Progress counts once toward the Register to Vote badge."}</span>
        </div>
      </MobileCard>
    </div>
  );
