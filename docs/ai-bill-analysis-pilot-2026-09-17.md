...
The deployed Details route renders a synced summary and source-based lens without waiting for a remote summary or model response. When no useful summary is stored, a separate streamed section checks Congress.gov after the page opens. On September 17, Tyler measured **6.49 seconds from a bill search result to H.R. 7008 Overview** and noticed that the CRS card omits the photo ID section. PR #41 added a notice that the CRS summary predates the House-passed version, linked the current text, removed Overview's unnecessary subscription lookup, and deferred secondary Congress.gov people data. A September 18 Production smoke check caught a regression: H.R. 7008's header showed "In Progress" while the official timeline included House passage and a newer August 6 Senate calendar action. PR #42 restored official actions on every tab, and PR #43 updated the page's latest action and status from that history. A live smoke check then found that refreshing the action broke the old action-only summary equality check: the stored July 23 action appeared as a "Stored Bill Summary" rather than yielding to the official CRS summary. [PR #44](https://github.com/Tylerandersongates/Capitol-Ledger/pull/44) keeps action-only placeholders identifiable after latest-action refresh and reached Production Ready/Current at merge `617a474`. Live Overview/Details show **On Floor**, Timeline starts with the August 6 Senate calendar action, and Details shows the official CRS summary and dated photo ID version notice. Physical-phone timing after that correction remains to be measured. The controlled live-check script still fetches a fresh summary for evaluation. An official-text packet and persistent AI cache remain separate pilot work.
