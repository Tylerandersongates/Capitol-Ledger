...
The current source boundary is `92b61b970423ce33dc32f6a2f0a9b72a6d3e2e86` on `codex/sept12-privacy-neon-candidate`; verified predecessor PR checkpoint `0c09abc0523e2f4de16f55f4d6d2465d27f4abfa`, exact-head [CI #321](https://github.com/Tylerandersongates/Capitol-Ledger/actions/runs/34780116093), matching Ready Vercel Preview deployment `7d1i8rinVS8iUhUTPUyba4DMwUfy`, and targeted privacy-fallback gate-off smoke provide source evidence. Verify the documentation-only packet descendant at its own live SHA. Production source remains `7ec68bcf142d6defe865c12959b0f9a84fce72d5` and still contains the older policy/source; the same source was redeployed Ready after the approved sender/contact environment update, and live `/privacy` smoke passed. The source-bound local recapture manifest is [prepared here](app-store-assets/capitolwonk-capture-manifest-2026-09-13.md).
...
3. Tyler selected Option 1 on September 12. Source candidate `92b61b970423ce33dc32f6a2f0a9b72a6d3e2e86` now routes `/support` and `/privacy` to a dedicated `/privacy/request` page/API, keeps the exact activation gate off by default, reads the fallback mailbox only from explicit runtime configuration, and removes Sentry as the privacy-rights route. Candidate branch `codex/sept12-privacy-neon-candidate` is pushed and unmerged in open [PR #8](https://github.com/Tylerandersongates/Capitol-Ledger/pull/8). Predecessor checkpoint `0c09abc0523e2f4de16f55f4d6d2465d27f4abfa` has passing exact-head [CI #321](https://github.com/Tylerandersongates/Capitol-Ledger/actions/runs/34780116093), matching Ready Vercel Preview deployment `7d1i8rinVS8iUhUTPUyba4DMwUfy`, and passing targeted `/privacy/request` smoke with first-party database intake disabled; verify the documentation-only packet descendant separately. The candidate has not been production-deployed, migrated, activated, or fully runtime/provider-evidenced.
...
2. Pin captures to the eventual exact release source. Current source candidate `92b61b9` and verified predecessor checkpoint `0c09abc` are non-production evidence only; rerun capture evidence after any source change.
