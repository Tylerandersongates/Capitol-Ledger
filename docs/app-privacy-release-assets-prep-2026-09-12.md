...
The current source boundary is `92b61b970423ce33dc32f6a2f0a9b72a6d3e2e86` on `codex/sept12-privacy-neon-candidate`; PR #8, CI #319, a matching Vercel Preview, and targeted privacy-fallback gate-off smoke provide evidence through handoff checkpoint `c96662a`. Production source remains `7ec68bcf142d6defe865c12959b0f9a84fce72d5` and still contains the older policy/source; the same source was redeployed Ready after the approved sender/contact environment update, and live `/privacy` smoke passed.
...
3. Tyler selected Option 1 on September 12. The candidate routes `/support` and `/privacy` to a dedicated `/privacy/request` page/API, keeps the exact activation gate off by default, and reads the fallback mailbox only from explicit runtime configuration. PR #8 through handoff checkpoint `c96662a` has passing [CI #319](https://github.com/Tylerandersongates/Capitol-Ledger/actions/runs/34777993751), a matching Vercel Preview, and targeted gate-off smoke. The candidate has not been production-deployed, migrated, activated, or fully runtime/provider-evidenced.
