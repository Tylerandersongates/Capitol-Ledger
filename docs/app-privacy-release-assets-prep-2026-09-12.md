...
The current source boundary is `92b61b970423ce33dc32f6a2f0a9b72a6d3e2e86` on `codex/sept12-privacy-neon-candidate`; PR #8, CI #314, a refreshed Ready Preview, and targeted privacy-fallback gate-off smoke provide evidence through documentation checkpoint `07e5b96`. Production source remains `7ec68bcf142d6defe865c12959b0f9a84fce72d5` and still contains the older policy/source; the same source was redeployed Ready after the approved sender/contact environment update, and live `/privacy` smoke passed.
...
3. Tyler selected Option 1 on September 12. Source candidate `92b61b970423ce33dc32f6a2f0a9b72a6d3e2e86` now routes `/support` and `/privacy` to a dedicated `/privacy/request` page/API, keeps the exact activation gate off by default, reads the fallback mailbox only from explicit runtime configuration, and removes Sentry as the privacy-rights route. Candidate branch `codex/sept12-privacy-neon-candidate`, through documentation checkpoint `07e5b96`, is pushed in [PR #8](https://github.com/Tylerandersongates/Capitol-Ledger/pull/8). [GitHub CI #314](https://github.com/Tylerandersongates/Capitol-Ledger/actions/runs/34719846610) passed every job, the refreshed Vercel Preview reported Ready, and targeted `/privacy/request` smoke displayed the verified privacy fallback while first-party database intake remained disabled. The candidate has not been production-deployed, migrated, activated, or fully runtime/provider-evidenced.
