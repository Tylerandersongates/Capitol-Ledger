...
Status: **Option 1 local privacy-intake source/copy work approved and implemented; no publication or upload is authorized.** The matrix and asset plan below are provisional until the new source is frozen and the exact signed Release archive, runtime/provider evidence, product configuration, and deletion/retention path are reconciled. This packet authorizes no production migration/deployment, provider/configuration mutation, mailbox creation, App Store Connect questionnaire edit, screenshot upload/replacement, signing, distribution, submission, or release.

The verified source boundary remains `3dbba3a260b10924dff254deed7f65a5e392c239`; documentation checkpoint `cd094530efacdbc3507579a0b52414aa7369d826` is synchronized on `codex/logo-refresh-sept10`. Exact-source CI `34668039916`, matching Ready Preview `6K9Xd24c4Pg4Nz1pRLorNzo6MxGb`, and recorded anonymous smoke remain evidence for that boundary. Production remains `7ec68bcf142d6defe865c12959b0f9a84fce72d5` and still contains the older policy/source.
...
3. Tyler selected Option 1 on September 12. Local source candidate `92b61b970423ce33dc32f6a2f0a9b72a6d3e2e86` now routes `/support` and `/privacy` to a dedicated, authenticated `/privacy/request` page/API, keeps the exact activation gate off by default, leaves the fallback mailbox blank until verified, and removes Sentry as the privacy-rights route. The candidate passed local source/build checks but has not been pushed, remotely CI/Preview-verified, deployed, configured, or runtime-evidenced.
4. The local source now implements minimized authenticated intake and account-visible status, and Tyler is recorded as the sole privacy owner. No backup is currently available. Access/export/correction/deletion-assistance fulfillment still lacks a documented single-owner contingency, verified mailbox/provider, secure export delivery, resolution tooling, monitoring, retention procedure, and completed synthetic operating exercise. Mailbox/contingency work is deferred to October 7–10 but remains a hard gate before the planned October 19 submission. Batch B remains blocked until those items are implemented or the public promises are narrowed.
...
The approved local copy changes move beyond verified candidate `3dbba3a`; they are not production evidence. The new working candidate must be frozen and pass fresh CI/Preview and runtime review before publication.
...
- Resend Pro documents 30-day email/log retention and seven-day backups. No messages appeared in the visible fifteen-day account view and no Resend webhooks were configured. This does not prove whether the write-only Vercel delivery modes are enabled.
- No exact configured destination was evidenced for auth, Team invite, Weekly Brief, or official-contact webhooks. Absent environment-name evidence supports “not evidenced,” not a claim that every external path is impossible.

## Existing Asset Audit

| Artifact | Exact technical evidence | Release finding |
| --- | --- | --- |
| `docs/app-store-assets/listing/capitolwonk-iphone-6.5-dashboard.png` | 1284 x 2778 PNG, sRGB RGBA, 2,889,527 bytes; SHA-256 `c5a8d918a3582162a2f06070e120df2229b52125af3ffbbdfcefcbd745b50007` | Visibly says `CAPITOLWONK CE`; must not be reused as the final CapitolWonk listing image. |
| `docs/app-store-assets/listing/capitolwonk-ipad-13-dashboard.png` | 2064 x 2752 PNG, sRGB RGBA, 2,480,970 bytes; SHA-256 `893f9d310610a690f5d061025005891194bb655d5fa9369b0d06920c859b281f` | Visibly says `CAPITOLWONK CE`; also shows native plus faux status chrome, a phone bezel/notch, and large black gutters at iPad width. |
