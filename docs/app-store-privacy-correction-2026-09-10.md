...
2. Audit the final production web path separately: Sentry JavaScript, hosting/request logs, database storage, authentication and Team-invite email, any Brief/email webhook, official-contact delivery, and any enabled embedded-video provider.
...
- Account-deletion production schema/FK, task-secret/scheduler/monitoring/retry, disposable-account, Team-member/legacy-Stripe cleanup, physical-device, backup/PITR, and provider-removal gates: pending.
