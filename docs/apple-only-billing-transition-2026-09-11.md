...
8. **Deletion/retention activation gate:** source candidate `92b61b9` defaults account deletion, privacy intake, and the retention sweep off, with legacy feedback behind its own additional switch. These controls are committed only in the isolated candidate; they are not merged, approved for production, deployed, configured, or activated and do not replace production/runtime proof.
9. **Migration gate:** the exact five-item candidate-to-production order in the [September 13 promotion packet](production-five-migration-promotion-packet-2026-09-13.md) has a fresh aggregate preflight, reviewed allowlist, backup/restore decision, partial-failure boundary, postflight, and separate action-time approval.
