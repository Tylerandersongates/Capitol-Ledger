...
- September 13 Neon gate: automatic child expiry, exact production-target reconciliation, empty-database cleanup, and post-delete read smoke completed
...
- Open QA: later constrained-restore drill phases, real PostgreSQL migration/runtime/cleanup behavior, forwarding/outbound delivery, privacy operations and single-owner contingency, Apple signing, native monitoring, physical-device behavior, Apple/Team sandbox lifecycle, and review/distribution.
...
- T09: source candidate, Neon Phase 1, automatic branch/compute cleanup, exact database-target reconciliation, approved empty-database hygiene, controlled-domain email, CI, Preview, and gate-off smoke are complete. Protected runtime/provider evidence, five-migration production packet, privacy operations, single-owner contingency, delivery exercises, corrected screenshots, App Privacy reconciliation, and every migration/deployment/activation decision remain open.
...
- September 12–14 milestone: implementation, candidate freeze, controlled-domain email refresh, exact-head CI, matching Preview, privacy fallback smoke, automatic Neon child/compute cleanup, final target reconciliation, and approved empty-database hygiene are complete. Next, continue the remaining protected-runtime, retention, and privacy-operations evidence without merging or activating the candidate.
...
## September 13 Completed Neon Cleanup And Next Best Steps

1. **Completed September 13:** the temporary Phase 1 branch and compute expired automatically; only protected `production` remains.
2. **Completed September 13:** final read-only reconciliation proved production reads use literal `Capitol%20Ledger`, while spaced-name `Capitol Ledger` had zero public tables and no application dependency. With separate exact confirmation, only that empty 7344 kB database was permanently deleted. `neondb` and `Capitol%20Ledger` remain, and post-delete production smoke passed.
...
> Continue CapitolWonk from `docs/eod-handoff-2026-09-12.md`, `docs/project-timeline.md`, and `docs/production-privacy-deletion-approval-packet-2026-09-11.md`. First confirm branch `codex/sept12-privacy-neon-candidate` is synchronized at the documented checkpoint and that exact non-production source candidate `92b61b970423ce33dc32f6a2f0a9b72a6d3e2e86` remains isolated and unmerged. Documentation checkpoint `64bc914` passes exact-head GitHub CI #317; the matching Vercel Preview is Ready, and exact `/privacy/request` smoke shows `privacy@capitolwonk.com` while database-backed intake remains disabled. Production remains `7ec68bc`. The September 13 Neon cleanup is complete for its approved scope: the temporary restore child and compute expired automatically, live production reads were verified on literal `Capitol%20Ledger`, and—with separate exact confirmation—only empty spaced-name `Capitol Ledger` was permanently deleted. Neon retains `neondb` and `Capitol%20Ledger`, and post-delete production smoke passed. NEXT SAFE ACTION: continue the remaining exact protected-runtime, retention, and privacy-operations evidence while keeping PR #8 unmerged and intake disabled. T03 remains gated by Apple issue #447, and T04 remains blocked pending Apple Support with the signing/device freeze intact. October 30 remains the user-set launch target but is low-confidence/materially at risk. Seek separate exact approval before any further database deletion, restore, migration, candidate deployment, intake activation, test message, provider mutation, App Store change, signing action, destructive QA, upload, distribution, submission, or release.
