...
> September 15 launch-priority and containment correction: PR #28 merged into `main` at `e045518` as documentation/CI-only evidence. The later principal-ACL inventory did not run and must not be retried. During its preparation the current Neon owner credential was exposed in local terminal output; the exact-target guard stopped on the wrong/default database, and no catalog ACL query or production database mutation occurred. Temporary client tooling was removed, the browser clipboard was cleared, and a narrow read-only Vercel inspection found no `DATABASE_URL` in project variables, no linked shared variables, and no connected database resource. This does not exclude an unknown external client. Tyler directly reset protected production `neondb_owner`; Neon confirmed success with the replacement masked, and Codex did not read, copy, download, or record it. Containment is closed and the [sanitized record](neon-owner-credential-containment-2026-09-15.md) is authoritative. The database-backed privacy operator sequence is conditional post-launch unless Tyler explicitly restores it to October 30 scope. Use the configured privacy mailbox for launch, keep every first-party privacy processing gate off, and move routine work to App Privacy/assets/metadata, provider/retention evidence, T08 scope, and the official Apple verifier-fix watch. The September 15 resumption section below is historical where this paragraph supersedes it.
...
Carry the Standing Rules above and the full timeline ledger. The protected production `neondb_owner` reset is complete, and the replacement was not disclosed or recorded. The ACL catalog read is retired and must not be retried. Continue routine Apple-independent launch preparation: App Privacy/listing metadata/corrected-wordmark assets, read-only provider/retention evidence, T08 scope, and the official verifier-fix watch. Use the dedicated privacy mailbox for launch. Keep all first-party privacy processing gates and the Apple T04 signing/device path off.
...
- **T09:** use the dedicated mailbox for launch and complete App Privacy/listing metadata/corrected-wordmark assets plus read-only provider/retention evidence. The first-party operator database sequence is conditional post-launch; its ACL inventory did not run and must not be retried. The exposed owner credential was reset and containment is complete. Every later production ACL/migration/role/credential/activation action remains an exact-approval checkpoint.
- **T10–T11:** TestFlight distribution, App Review submission, and launch remain unapproved and dependent on the upstream gates.

Preserve the October 2–6 buffer and October 20–29 contingency. The existing estimate remains `7–13` hands-on working days excluding external waits, re-review, remediation, and QA fixes. October 30 remains low-confidence and materially at risk while Apple, verifier, signing, device, sandbox, App Privacy/assets, distribution, and review gates remain open. The deferred privacy-operator runtime is no longer counted as a launch dependency.
