...
Carry the Standing Rules above and the full timeline ledger. First, Tyler completes the protected production `neondb_owner` reset directly in Neon and reports only completion; do not record the replacement value. The ACL catalog read is retired and must not be retried. Then continue routine Apple-independent launch preparation: App Privacy/listing metadata/corrected-wordmark assets, read-only provider/retention evidence, T08 scope, and the official verifier-fix watch. Use the dedicated privacy mailbox for launch. Keep all first-party privacy processing gates and the Apple T04 signing/device path off.
