...
Status: **specification approved for preparation and complete; implementation remains unapproved.** This document converts the September 12 `jsrsasign` risk decision into a deterministic test contract and an exact fallback comparison. It changes no package, lockfile, source, test, deployment, provider setting, Apple configuration, signed build, upload, distribution, submission, or release. The production deployment and activation blocks in the [risk decision packet](jsrsasign-risk-decision-2026-09-12.md) remain in force.
