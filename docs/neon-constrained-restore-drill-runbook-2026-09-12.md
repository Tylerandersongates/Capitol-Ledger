...
Status: **the constrained restore-floor drill is complete through Phases 1–6; production migration and restore remain unapproved.** Tyler separately approved the constrained restore floor, isolated branches, the five-migration and compatibility drill, persistent branch-only controls, the synthetic deletion, historical comparison, and final cleanup. CapitolWonk must never resume traffic from a database point earlier than the newest verified completed account deletion. If the newest trustworthy deletion watermark cannot be obtained or preserved for comparison, service remains offline. The pre-deletion point returned `REJECT`, the post-deletion point returned `ELIGIBLE FOR FURTHER RECOVERY REVIEW`, and the exact replacement child and its compute have been removed. No production migration, production restore/reset, traffic switch, source deployment, provider effect, or persistent comparison resource occurred.
...
During Phases 2–5, the raw control locators existed only at `/private/tmp/codex-neon-restore-controls-20260913.json`, permission mode `0600`, with no database URL; the sanitized control result was also mode `0600`. The clipboard was cleared, the one-shot scripts and server log were removed, the local port was closed, and no `.env` file was created or downloaded. Both temporary control files were later destroyed during the separately approved Phase 6 cleanup recorded below.
...
The preserved time relation is **`T-pre = 2026-09-13T22:26:52.250Z < T-delete = 2026-09-13T22:50:14.926Z < T-post = 2026-09-13T22:50:20.969Z`**. The sanitized result was held only in mode-`0600` temporary storage through comparison; raw account/session/request locators remained excluded from repository evidence. The clipboard was cleared, no `.env` was created or downloaded, and the local server was stopped. Those temporary files were later destroyed during the separately approved Phase 6 cleanup.
...
Both historical points proved PostgreSQL 17, `public`, 33 public base tables, 18 migration-history rows comprising 17 successes and one resolved rollback, and identical protected totals: 564 members, 17,941 bills, 1,515 votes, 365,830 member votes, 7 petition signatures, 2 Team pauses, and zero official contacts, App Store states/receipts, or privacy requests. No production or application traffic was sent, no provider was invoked, and no persistent comparison resource required cleanup. The original isolated child and protected temporary evidence were later removed in Phase 6.
...
### Phase 6 cleanup evidence — September 13, 2026

Tyler explicitly approved the destructive Phase 6 cleanup in the controlling Codex task. The scope was limited to deleting isolated child `phase2-five-migration-20260913` (`br-rapid-glitter-ak37qyuf`) and its compute, verifying the remaining Neon inventory, destroying the drill's protected temporary control/comparison evidence, and updating this packet. It did not authorize any action against protected/default `production`, a production database write, restore, traffic or provider change, deployment, merge, or gate activation.

Neon's branch-specific confirmation permanently deleted only the named child. By **`2026-09-13T23:51:32Z`**, the branch inventory had returned from two branches to the single protected/default `production` branch. The production Computes view showed only its suspended primary endpoint `ep-dry-thunder-aktc1o54` at `0.25 ↔ 8 CU`, with no read replica; the deleted child's primary compute was absent. No drill deployment or provider configuration had been attached, and no persistent comparison branch had been created.

The exact mode-`0600` files `/private/tmp/codex-neon-restore-controls-20260913.json`, `/private/tmp/codex-neon-restore-controls-result.json`, and `/private/tmp/codex-neon-restore-comparison-result.json` were permanently removed and verified absent. The empty `/private/tmp/codex-neon-cli-phase6-20260913` directory from an attempt stopped before fresh reauthentication consent was also removed; no new OAuth grant was completed. No `.env` file was created or downloaded. Sanitized durable evidence remains only in the reviewed repository documentation.

Phase 6 is complete. The branch/compute inventory is back at baseline, there is no retained drill credential or locator file, and production remains untouched and protected.
...
Current result: **Phases 1–6 are complete and the constrained restore-floor drill is closed.** The replacement child `phase2-five-migration-20260913` was derived from protected `production`, used literal application database `Capitol%20Ledger`, and had no attached application traffic or outbound integration. Corrected preflight, all five migrations, full postflight, application reads, no-provider compatibility reproduction, persistent-control creation, the exact deletion route, and both historical comparison gates passed. The pre-deletion point returned `REJECT`; the post-deletion point returned `ELIGIBLE FOR FURTHER RECOVERY REVIEW`; the survivor was unchanged; and schema, migration history, cleanup state, and protected aggregates matched. The child and its compute are deleted, the Neon inventory contains only protected/default `production` and its primary compute, and all drill-only temporary control/comparison files are absent. No restore, traffic switch, production action, provider call, deployment, or persistent comparison branch occurred. Production remains untouched and protected. This closes the recovery-proof prerequisite only; it does not authorize Batch A.

The September 14 [durable verifier checkpoint](privacy-artifact-deletion-and-restore-floor-verifier-2026-09-14.md) replays only the sanitized `T-pre`/`T-delete`/`T-post` relationship and closed invariants in memory. It has no Neon/database/provider binding and does not repeat or broaden this completed exercise.
