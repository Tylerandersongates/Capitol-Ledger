...
Status: **restore-floor policy and Phase 1 branch/read-only execution approved and complete; the exact application database, migration state, and zero-row deletion-watermark baseline are verified.** Tyler approved the constrained restore floor, one isolated child branch, the read-only deployment-target reconciliation, and aggregate-only checks on September 12. CapitolWonk must never resume traffic from a database point earlier than the newest verified completed account deletion. If the newest trustworthy deletion watermark cannot be obtained or preserved for comparison, service remains offline. Phase 1 found no completed-deletion watermark because the deletion-request table has zero rows. No branch-only migration, fixture, synthetic deletion, restore/reset, traffic switch, provider cleanup, credential creation/reset/persistence, or manual branch deletion is authorized.
...
Phase 1 is complete. The drill now stops at the fresh five-migration/fixture approval boundary before any migration, row write, synthetic deletion, or provider effect. The temporary branch is left to its one-day automatic expiry; verify its disappearance after the displayed deadline. Manual deletion still requires destructive action-time confirmation.

Production database hygiene is intentionally deferred until the temporary branch's automatic expiry is verified. A one-time read-only follow-up is scheduled for September 13 at 12:00 PDT, after the displayed expiry. It may report success or a cleanup discrepancy but must not delete anything. After automatic expiry is verified, repeat the read-only dependency and target checks, retain `neondb` and the literal application database `Capitol%20Ledger`, and present the empty spaced-name `Capitol Ledger` database for a separate permanent-delete confirmation. Do not combine that cleanup with a migration or restore action.
...
Current result: **Phase 1 complete; drill open at the Phase 2 approval boundary.** The application database, five-item pending migration set, and `no completed-deletion watermark present` baseline are verified. Phases 2–5 were not authorized or attempted, and automatic branch cleanup remains to be verified.
