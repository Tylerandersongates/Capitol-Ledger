...
Status: **restore-floor policy, Phase 1, and the Phase 2 five-migration/schema component are verified complete; persistent restore controls and Phases 3–5 remain open.** Tyler approved the constrained restore floor, the original isolated child/read-only reconciliation, and then a separately scoped replacement-child migration and compatibility drill. CapitolWonk must never resume traffic from a database point earlier than the newest verified completed account deletion. If the newest trustworthy deletion watermark cannot be obtained or preserved for comparison, service remains offline. The replacement child `phase2-five-migration-20260913` now holds the reviewed five migrations with clean preflight/postflight evidence, zero pending migrations, unchanged protected aggregates, and no public traffic or provider effect. The compatibility fixture was temporary and fully removed; it was not either of the persistent restore controls required below. No synthetic account deletion, historical comparison branch/view, production migration, production restore/reset, traffic switch, source deployment, or child cleanup occurred under that approval.
...
Current result: **Phase 1 and the Phase 2 migration/schema component are complete; the drill is open at the persistent-control approval boundary.** The replacement child `phase2-five-migration-20260913` was derived from protected `production`, uses literal application database `Capitol%20Ledger`, expires automatically on September 14, 2026 at 14:04:45 PDT, and has no attached application traffic or outbound integration. Corrected preflight, all five migrations, Prisma status, full postflight, application reads, and no-provider compatibility reproduction passed; the temporary compatibility fixture was removed. The persistent deletion-marker and survivor accounts do not exist, `T-pre` has not been recorded, and no destructive deletion or comparison branch/view has been authorized or attempted. Production remains untouched and protected.
