...
The next local checkpoint adds the [operator-boundary and pause/return packet](privacy-request-operator-boundary-and-pause-runbook-2026-09-14.md), its machine-readable contract, and a pure guard for the future server-only runner. The packet specifies the exact Production-scoped Vercel pause/return procedure and validates the two-window decision locally, but it does not preauthorize either configuration change. No executable runner or production binding exists, so the production-binding, migration, provider, scheduling, live-data, retention, and activation gaps above remain open.
