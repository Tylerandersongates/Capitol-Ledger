...
-- ownership and validity, foreign-
-- key targets/actions, sequence parameters, and aggregate integrity. The
-- detailed defaults, CHECK expressions, FK column mappings, and index keys and
-- predicates printed after the DO block still require a line-by-line manual
-- comparison with the reviewed migration SQL before postflight can close.
...
BEGIN
  ...
  IF missing_or_invalid_constraints <> 0 THEN
    RAISE EXCEPTION 'STOP: % expected constraints have the wrong owner/type/validation state', missing_or_invalid_constraints;
  END IF;

  SELECT count(*) INTO missing_or_invalid_foreign_keys
  FROM (VALUES
    ('AccountDeletionRequest', 'AccountDeletionRequest_userId_fkey', 'User', 'n', 'c'),
    ('OfficialContactMessage', 'OfficialContactMessage_userId_fkey', 'User', 'c', 'c'),
    ('PetitionSignature', 'PetitionSignature_userId_fkey', 'User', 'c', 'c'),
    ('TeamSubscriptionPause', 'TeamSubscriptionPause_userId_fkey', 'User', 'c', 'c'),
    ('TeamSubscriptionPause', 'TeamSubscriptionPause_workspaceId_fkey', 'TeamWorkspace', 'c', 'c'),
    ('AccountDeletionCleanupJob', 'AccountDeletionCleanupJob_deletionRequestId_fkey', 'AccountDeletionRequest', 'c', 'c'),
    ('AppStoreSubscriptionState', 'AppStoreSubscriptionState_userId_fkey', 'User', 'c', 'c'),
    ('AppStoreNotificationReceipt', 'AppStoreNotificationReceipt_userId_fkey', 'User', 'n', 'c'),
    ('PrivacyRequest', 'PrivacyRequest_userId_fkey', 'User', 'c', 'c')
  ) expected(table_name, constraint_name, referenced_table, delete_action, update_action)
  WHERE NOT EXISTS (
    SELECT 1
    FROM pg_constraint constraint_record
    JOIN pg_class table_record ON table_record.oid = constraint_record.conrelid
  ...
  IF missing_or_invalid_foreign_keys <> 0 THEN
    RAISE EXCEPTION 'STOP: % expected foreign keys have the wrong target/action/validation state', missing_or_invalid_foreign_keys;
  END IF;
  ...
  FROM (VALUES
    ...
    ('AccountDeletionCleanupJob', 'AccountDeletionCleanupJob_dedupeKey_key', TRUE),
    ('AccountDeletionCleanupJob', 'AccountDeletionCleanupJob_status_availableAt_idx', FALSE),
    ('AccountDeletionCleanupJob', 'AccountDeletionCleanupJob_deletionRequestId_idx', FALSE),
    ('AppStoreSubscriptionState', 'AppStoreSubscriptionState_userId_key', TRUE),
    ('AppStoreSubscriptionState', 'AppStoreSubscriptionState_appAccountToken_key', TRUE),
    ('AppStoreSubscriptionState', 'AppStoreSubscriptionState_originalTransactionId_key', TRUE),
    ('AppStoreSubscriptionState', 'AppStoreSubscriptionState_environment_appleStatus_idx', FALSE),
    ('AppStoreSubscriptionState', 'AppStoreSubscriptionState_reconciledAt_idx', FALSE),
    ('AppStoreNotificationReceipt', 'AppStoreNotificationReceipt_notificationUUID_key', TRUE),
    ('AppStoreNotificationReceipt', 'AppStoreNotificationReceipt_status_createdAt_idx', FALSE),
    ('AppStoreNotificationReceipt', 'AppStoreNotificationReceipt_userId_createdAt_idx', FALSE),
    ('PrivacyRequest', 'PrivacyRequest_userId_requestedAt_idx', FALSE),
    ('PrivacyRequest', 'PrivacyRequest_status_requestedAt_idx', FALSE),
    ('PrivacyRequest', 'PrivacyRequest_one_active_type_per_user_idx', TRUE)
  ...
  IF missing_or_invalid_indexes <> 0 THEN
    RAISE EXCEPTION 'STOP: % expected indexes have the wrong owner/type/uniqueness/validity state', missing_or_invalid_indexes;
  END IF;
...
SELECT 'MANUAL_COMPARE: exact PK/FK/CHECK definitions and column mappings' AS required_review,
...
SELECT 'MANUAL_COMPARE: exact index keys/order/predicate in addition to machine-checked owner/flags' AS required_review,
       table_record.relname AS table_name,
