...
BEGIN
  ...
  -- The successful row for the known 20260618162000 retry was recorded by
  -- Prisma resolution after the original attempt was rolled back, so its
  -- frozen applied_steps_count is exactly 0. Every ordinary successful row
  -- remains pinned to 1; this is not a general relaxation of the step gate.
  ...
  -- must be a live/ready/valid index on the intended table with the intended
  -- uniqueness, key order, and predicate presence. Exact predicate expressions
  -- and complete definitions remain a manual comparison.
  ...
  IF unsafe_existing_indexes <> 0 THEN
    RAISE EXCEPTION 'STOP: % existing migration-1 index names have unsafe ownership/type/keys/predicate-presence/flags', unsafe_existing_indexes;
  END IF;
...
-- Save this complete inventory and compare it to the postflight inventory as a
-- set. Counts alone cannot detect one old relation disappearing while an
-- unrelated relation is added.
