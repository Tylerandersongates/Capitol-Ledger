...
- **Next safe action:** finish the remaining isolated recovery prerequisites while the replacement child exists. First obtain exact approval to create the persistent deletion-marker and survivor controls and record `T-pre`; stop before deletion. Separately prove the proposed Prisma migration-session timeouts on a disposable descendant, or retain the timeout gap. Then obtain action-time approval for the destructive synthetic deletion and pre/post comparison branches. Do not request production migration approval until recovery proof and the production compatibility sequence pass.
