...
| --- | --- |
| Approval reference and scope | Tyler confirmed the replacement-child migration/compatibility action, then separately approved persistent restore controls, and separately confirmed the destructive Phase 3 deletion-marker action in the controlling Codex task on September 13, 2026. Phase 3 was limited to one exact deletion-route invocation on the synthetic marker, sanitized verification, and `T-delete`/`T-post`; comparison branches/views and child cleanup remained outside scope. Production, public traffic, provider mutation, source deployment, activation, destructive failure injection, and child deletion were outside scope and did not occur. |
| UTC start/end; operator/reviewer | Corrected migration preflight began by `2026-09-13T21:42:19Z`; final post-fixture reconciliation completed at `2026-09-13T21:53:36Z`; Phase 3 recorded `T-delete = 2026-09-13T22:50:14.926Z` and stable `T-post = 2026-09-13T22:50:20.969Z`. Codex operated the isolated drill under Tyler's action approvals. There was no separate live lock observer; prior local packet review and exact-head CI/Preview review covered the frozen inputs. |
| Candidate and production SHA | Phase 3 action/PR checkpoint `c9517846a207a9071967304b0fc4c6806dbafd18`; frozen source candidate `92b61b970423ce33dc32f6a2f0a9b72a6d3e2e86`; unchanged production source `7ec68bcf142d6defe865c12959b0f9a84fce72d5`. The action checkout and PR head were clean and synchronized before the drill. |
...
| Target guard and Prisma status | Target guard passed for literal `Capitol%20Ledger` / `public`. Before migration Prisma 5.22 found 17 repository migrations and only the reviewed five pending in order. After migration, again after fixtures, and both before and after Phase 3 deletion, Prisma reported the schema up to date with zero pending. |
...
| Restore-deletion checkpoint | Under separate destructive approval, a local production build called same-origin `POST /api/account/deletion-request` exactly once for only the synthetic deletion marker, with the deletion gate locally enabled and every unrelated delivery/provider credential or mode disabled. A pre-route stop corrected fixture-only versus inherited-global count semantics; it performed no POST or deletion. The successful route returned a completed/resolved receipt. Marker account/session/token and all checked attributable linked rows are absent; the receipt is resolved and deidentified; exactly one qualifying watermark exists; cleanup jobs remain zero; and the survivor hash is unchanged. Inherited global deltas were exactly 86→85 users, 3→2 sessions, 85→84 verification tokens, and 0→1 deletion requests. `T-pre = 2026-09-13T22:26:52.250Z`, `T-delete = 2026-09-13T22:50:14.926Z`, and `T-post = 2026-09-13T22:50:20.969Z`. No comparison branch/view or child cleanup occurred. |
| Discrepancy/recovery decision | No migration, catalog, checksum, aggregate, smoke, deletion-postcondition, or cleanup discrepancy remains. The Phase 3 runner's initial fixture/global-count mismatch stopped before its POST and was corrected with inherited-global delta preservation. The expected legacy-source incompatibility was positively reproduced, so production Batch A remains no-go while `7ec68bc` can receive delayed/retried Team checkout events. The isolated evidence supports review of the compatible-source-first option but does not approve it; choose and separately approve either that sequence or complete checkout/webhook quiescence plus delayed-event reconciliation. Restore comparison Phases 4–5, cleanup Phase 6, runner timeout propagation, and fresh production evidence remain open. |
...
- Persistent restore-control creation and the destructive synthetic deletion are complete under their separate approvals. Historical comparison branches/views and temporary-resource cleanup remain separately approved steps. The completed compatibility fixture was temporary and is not a persistent restore control.
