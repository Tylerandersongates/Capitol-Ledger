...
Status: **documentation and read-only verification preparation only; production migration is not approved and is not ready for approval.** Exact source candidate `92b61b970423ce33dc32f6a2f0a9b72a6d3e2e86` remains isolated and unmerged on `codex/sept12-privacy-neon-candidate`. The last independent September 13 remote verification covered predecessor documentation/SQL checkpoint `0c09abc0523e2f4de16f55f4d6d2465d27f4abfa`; exact-head [CI #321](https://github.com/Tylerandersongates/Capitol-Ledger/actions/runs/34780116093) passed and the matching [Vercel Preview](https://vercel.com/capitolwonkce/project-qosv1/7d1i8rinVS8iUhUTPUyba4DMwUfy) reached Ready. The documentation-only checkpoint containing this file descends from that predecessor and carries the hardened SQL hashes below; CI #321 does not validate those later SQL bytes. Before any isolated or production action, prove the live PR head contains this exact committed tree and verify its own exact-head checks. Production remains `main` at `7ec68bcf142d6defe865c12959b0f9a84fce72d5`.
...
- **Next safe action:** require the live PR head to contain this reviewed documentation-only packet/SQL revision and pass its own exact-head checks. After that gate passes, separately approve creation of a replacement automatically expiring Neon child/compute, including exact parent/restore point, protected credential handling, fresh baseline, expiry, and cleanup owner. Execute the five migrations there, run the full postflight, and exercise the production-source compatibility boundary with local/mocked Stripe fixtures before choosing a production sequence. Do not request production migration approval until that evidence and the recovery prerequisites pass.
- **Why:** Phase 1 verified the exact target/schema and five-item pending set, but the five SQL files have not been executed together on a real PostgreSQL clone. There is no down migration or tested rollback. The files contain DDL and conditional data cleanup without an explicit all-five transaction boundary. More importantly, production `7ec68bc` can write `workspaceId = 'team-owner-upgrade'` from a legacy Team `checkout.session.completed` event; migration 3 replaces that sentinel convention with a foreign key to `TeamWorkspace`. After migration, the old write fails after a possible Stripe cancellation attempt, and its webhook caller swallows the error. A brief database writer pause during the command does not resolve delayed or retried events after the pause.
...
| Verified predecessor documentation/SQL checkpoint | `0c09abc0523e2f4de16f55f4d6d2465d27f4abfa` (CI #321 and Preview evidence apply to this predecessor; verify the later packet checkpoint separately) |
...
- `docs/production-five-migration-preflight-2026-09-13.sql`: `64ecb4d45c068345ae91f5219dd659314eb733f9848140bd36d6c29cdf388b90`
- `docs/production-five-migration-postflight-2026-09-13.sql`: `929d9f2dc338a6c92cbe443f0bc91d37d375d3bae8417871450b15b83e9d9713`
...
1. **Exact packet checkpoint:** independent local review and hash reconciliation are complete in the documentation-only checkpoint containing this file. Before any action, verify the live PR head contains that exact tree and rerun proportionate exact-head checks; predecessor `0c09abc` does not contain every hardening change recorded here. Do not use a mixed revision, an untracked file, or a dirty/detached checkout for an isolated or production action.
2. **Replacement isolated resource and real-PostgreSQL execution:** obtain a separate exact approval to create a new child/compute from the named protected parent/restore point, with the approved secret-injection path, automatic expiry, cleanup owner, and no public traffic. Recheck target/baseline, run the prepared [preflight SQL](production-five-migration-preflight-2026-09-13.sql), apply only the five migrations, and run the [postflight SQL](production-five-migration-postflight-2026-09-13.sql). Record durations, observed locks/timeouts, history state, and sanitized results. Do not inject a destructive mid-migration failure under this approval; failure injection needs its own reviewed scope.
3. **Production-source compatibility decision:** reproduce the `7ec68bc` Team owner-upgrade sentinel failure safely with no external Stripe mutation, then prove one separately reviewed production sequence. Either deploy a minimal pre-migration-compatible source change that retires the sentinel path, or quiesce legacy Team checkout creation and webhook ingress/retries for the whole migration-to-compatible-source interval with a reconciliation plan for delayed events. A command-window-only write pause is insufficient. Provider configuration, traffic controls, source deployment, and delayed-event handling each require their stated approval; do not infer a preferred option from this packet.
...
5. **Concurrency/change-window plan:** measure the isolated run, choose action-time `lock_timeout` and `statement_timeout` values from evidence, prevent overlapping deploy/schema jobs, and decide whether matching writes need a maintenance window. The migrations create non-concurrent indexes, replace constraints, and may delete/update rows. Prove how those settings reach Prisma's migration connection; do not assume settings in a separate SQL client carry over. Use a separately authorized read-only observer if lock evidence is required.
6. **Fresh production preflight:** verify non-secret provider identity, protected branch, endpoint label, target guard, the exact 12-success/one-resolved-retry migration-history baseline and checksums, existing object definitions, complete public relation inventory, duplicates/orphans/sentinels, table sizes/counts, and unaffected catalog aggregates. Any legitimate baseline change requires a new reviewed packet/SQL checkpoint; it is not an operator override.
...
- Replacement isolated Neon child/compute creation, Phase 2 migration/fixtures, compatibility reproduction, and temporary-resource cleanup are separately approved steps.
