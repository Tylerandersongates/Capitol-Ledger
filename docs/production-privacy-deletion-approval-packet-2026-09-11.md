...
Status: **reconciled through September 13; source candidate `92b61b970423ce33dc32f6a2f0a9b72a6d3e2e86` supersedes the historical `3dbba3a` evidence boundary, and no migration, candidate deployment, or activation is authorized.** Candidate branch `codex/sept12-privacy-neon-candidate` freezes the approved Option 1 source/schema/test work, fifth migration, and database-target guard. Its offline frozen install, production/full registry audits, local release-source suite, Prisma validation/generation, strict TypeScript, lint, and production build pass; the strict protected-runtime wrapper correctly fails because deployment/App Store values are absent. The branch is isolated in open, unmerged [PR #8](https://github.com/Tylerandersongates/Capitol-Ledger/pull/8). Verified predecessor checkpoint `0c09abc0523e2f4de16f55f4d6d2465d27f4abfa` has passing exact-head [CI #321](https://github.com/Tylerandersongates/Capitol-Ledger/actions/runs/34780116093), matching [Vercel Preview `7d1i8rinVS8iUhUTPUyba4DMwUfy`](https://vercel.com/capitolwonkce/project-qosv1/7d1i8rinVS8iUhUTPUyba4DMwUfy), and passing targeted privacy-fallback gate-off smoke; verify the later documentation-only packet descendant at its own live SHA. Production remains unchanged at `main` source `7ec68bcf142d6defe865c12959b0f9a84fce72d5`. Separately, the approved Neon Phase 1 check found the exact application schema in literal `Capitol%20Ledger`, verified the five-item pending migration set, and confirmed `PrivacyRequest` absent, zero `AccountDeletionRequest` rows, and no completed-deletion watermark. The temporary child and compute expired automatically. A final read-only check then verified live production `Bill`, `Vote`, and `MemberVote` SELECTs on `Capitol%20Ledger`; spaced-name `Capitol Ledger` had zero public tables and no application dependency. With Tyler's separate exact confirmation, only that empty 7344 kB database was permanently deleted. Neon retains `neondb` and `Capitol%20Ledger`, and post-delete production smoke passed. Resend verifies `capitolwonk.com`; Porkbun forwards the `info`, `privacy`, `accounts`, and `briefs` aliases to the owner-selected external inbox, and Vercel Production/Preview hold the approved sender/contact values without changing secrets. The current `main` source was redeployed Ready after that configuration update and live `/privacy` smoke passed. Production Batch A is no-go because unchanged `7ec68bc` may still write legacy Stripe Team sentinel `team-owner-upgrade`, which migration 3 would make foreign-key-invalid; isolated mocked reproduction and a separately approved compatible-source or complete source/traffic-quiescence sequence are prerequisites. No candidate merge, production migration, privacy-intake activation, account deletion, restore, branch-only write, test message, App Store Connect change, signed build, upload, distribution, submission, or release occurred. The exact Batch A preparation now lives in the [September 13 five-migration promotion packet](production-five-migration-promotion-packet-2026-09-13.md).
...
| --- | --- |
| Current source candidate | `92b61b970423ce33dc32f6a2f0a9b72a6d3e2e86` on `codex/sept12-privacy-neon-candidate`; frozen in open, unmerged PR #8. Verified predecessor checkpoint `0c09abc0523e2f4de16f55f4d6d2465d27f4abfa` has exact-head CI #321, matching Ready Preview `7d1i8rinVS8iUhUTPUyba4DMwUfy`, and passing targeted privacy-fallback gate-off smoke; verify the documentation-only packet descendant at its own live SHA. Historical checkpoint `c96662a` / CI #318 remains preserved but is superseded by those later checkpoints. The candidate preserves the locked dependency graph and adds the approved privacy intake, fifth migration, corrected copy/routing, account-deletion postcondition, and database-target guard. Offline frozen install, both registry-backed audits, local-preparation release-source checks, Prisma validation/generation, strict TypeScript, lint, and production build pass. The strict release wrapper remains blocked on absent protected runtime/App Store values. No merge, candidate production deployment, migration, or activation has occurred. |
...
| Current production source | `main` at `7ec68bcf142d6defe865c12959b0f9a84fce72d5`; same-source environment refresh redeployed Ready after the approved sender/contact update; live `/privacy` smoke passed |
...
The migration command must be allowed to apply only this exact five-item pending set in lexical order. Do not rerun the already-applied deletion-request migration manually, edit `_prisma_migrations`, or suppress the Team-pause migration's orphan exception. Do not infer either database-first or source-first sequencing: unchanged `7ec68bc` is incompatible with migration 3's post-migration foreign key, while any source or traffic change is separately gated. First reproduce the boundary in the isolated mocked drill and approve one exact compatibility sequence.
...
This closes the source-design gap only. The gates are committed and locally source/build-verified; predecessor PR checkpoint `0c09abc`, exact-head CI #321, matching Ready Preview `7d1i8rinVS8iUhUTPUyba4DMwUfy`, and targeted privacy-fallback gate-off smoke provide time-stamped source evidence, while the documentation-only packet descendant requires separate exact-head verification. The candidate is not merged, production-deployed, activated, sandbox-tested, device-tested, or fully runtime/provider-verified. Before requesting Batch B, close the known `jsrsasign` issue #447 gate and obtain approval for the exact default-off deployment and later activation sequence. Deletion and retention must remain unavailable until the live schema, authenticated cleanup worker, no-payload monitoring, provider-retention procedures, sandbox/device behavior where applicable, and truthful public copy are ready. Deployment approval is not activation approval.

### A. Production migrations — no-go; pending separate explicit approval

Unchanged production source `7ec68bc` can still handle a legacy Stripe Team `checkout.session.completed` event by attempting the owner-upgrade flow and writing `workspaceId = 'team-owner-upgrade'`. Migration 3 converts existing occurrences to `NULL` and then adds a foreign key to `TeamWorkspace`, so a delayed or retried old event can fail after the migration; the caller can swallow that error after a possible Stripe cancellation attempt. A command-window-only writer pause does not close this delayed-event boundary. Before any Batch A approval request, use the separately approved replacement child/Phase 2 to reproduce the failure with every Stripe effect mocked or disabled, then validate one separately reviewed compatible-source-first sequence or a complete source/traffic-quiescence and delayed-event reconciliation sequence. This compatibility gate is additional to every prerequisite below.

The [September 13 five-migration promotion packet](production-five-migration-promotion-packet-2026-09-13.md) now holds the immutable manifest, executable read-only pre/postflight SQL, isolated-drill prerequisite, concurrency gap, and partial-failure decision tree. Batch A remains blocked until those prerequisites pass. Immediately before any later action:
...
- preserve the setting-verified seven-day Neon window and protected default `production` branch; Phase 1 of the [restore drill](neon-constrained-restore-drill-runbook-2026-09-12.md) verified the literal-name application schema, migration state, and zero-row deletion-watermark baseline, while later drill phases remain prerequisites before production migration approval; automatic child cleanup, final target reconciliation, and empty-database hygiene are complete;
...
Only after the compatibility sequence and every other gate pass may a separate proposal request one production `prisma migrate deploy` from the frozen candidate. Postflight must show all five migrations applied and no other migration changed. Then verify table/column/index/foreign-key definitions, including the privacy-request table, both App Store state tables, and their ownership/deidentification constraints; zero unexpected orphans; both existing Team-pause rows still present and workspace-valid; and unchanged non-account public catalog counts. This batch authorizes no unlisted source or traffic change, application deployment, Apple notification activation, privacy-intake activation, or account deletion.

### B. Matching production source deployment — pending separate explicit approval

Batch B is blocked until the known `jsrsasign` OCSP defect and residual EOL gate are closed through the approved path; a tested default-off App Store server-verifier/Notifications V2 gate and fail-closed rollback exist; the remaining Neon restore-drill phases are reconciled; the migration/source compatibility sequence is proven and approved; and an operated privacy-rights procedure exists for every retained access/export/correction/deletion promise. Automatic child cleanup, final production-target reconciliation, and empty-database hygiene are complete. Option 1 is selected and frozen in source candidate `92b61b9`, Tyler is the sole privacy owner, and the unsupported Sentry/deletion copy is corrected. The controlled-domain mailbox/provider prerequisite is now configured: Resend verifies the domain, Porkbun holds the four owner-directed aliases, and Vercel Production/Preview hold the sender/contact values. No test message was sent and no secret was rotated. No backup is currently available; the [single-owner contingency](privacy-operations-single-owner-contingency-2026-09-13.md) is prepared but not approved or operated, and forwarding/outbound delivery exercises, staff resolution path, export/correction/deletion exercises, monitoring, retention, authenticated/runtime-provider evidence, and separate candidate-deployment approval remain open. Open PR #8 head `0c09abc`, exact-head CI #321, matching Ready Preview `7d1i8rinVS8iUhUTPUyba4DMwUfy`, copy/readiness checks, and targeted privacy-fallback gate-off smoke provide current remote evidence for `92b61b9`; the strict protected-runtime gate remains intentionally unexecuted without its protected values. A candidate deployment approval would still not activate privacy intake, account deletion, retention, or App Store server processing.
...
- unchanged `7ec68bc` can still receive, retry, or process a legacy Team checkout-completion event after migration 3, or the exact compatible-source/traffic sequence and delayed-event reconciliation state cannot be proven;
...
- the expected five-migration allowlist/order changes, a checksum or source graph differs, or Prisma reports drift/incomplete history;
...
- redacted migration preflight/deploy/postflight with the exact five migrations;
...
| Batch | Status |
| --- | --- |
| Read-only production migration-history and aggregate safety preflight | Historical four-item production preflight complete September 11; five-item Phase 1 child baseline complete; fresh direct-production five-item preflight still required |
| A — production migrations | **No-go and not authorized.** Follow the [September 13 five-migration packet](production-five-migration-promotion-packet-2026-09-13.md). A replacement-child mocked reproduction and separately approved compatible-source or complete source/traffic-quiescence sequence are required because unchanged `7ec68bc` can write foreign-key-invalid `team-owner-upgrade` after migration 3. Restore-drill phases 2–5, isolated verifier/failure validation, runner/endpoint binding, lock/window and recovery plans, and fresh production evidence also remain open. |
...
| Default-off deletion, privacy-intake, and retention source gates | Preserved and extended in current candidate `92b61b9`, source-verified, and exact-CI verified through open PR head `0c09abc` / CI #321; not approved, deployed, activated, or runtime-verified |
| B — gated production source deployment | Blocked by the `jsrsasign` correctness/residual-EOL gates, the approved migration/source compatibility sequence, Batch A, actionable privacy-rights intake/procedure, truthful deletion/feedback copy, and activation-sequence approval; not authorized |
