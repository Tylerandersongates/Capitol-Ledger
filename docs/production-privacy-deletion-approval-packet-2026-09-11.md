...
Batch B is blocked until the known `jsrsasign` OCSP defect and residual EOL gate are closed through the approved path; a tested default-off App Store server-verifier/Notifications V2 gate and fail-closed rollback exist; the remaining Neon restore-drill phases and cleanup are reconciled; and an operated privacy-rights procedure exists for every retained access/export/correction/deletion promise. Option 1 is selected and frozen in source candidate `92b61b9`, Tyler is the sole privacy owner, and the unsupported Sentry/deletion copy is corrected. No backup is currently available; the single-owner contingency, verified mailbox/provider, staff resolution path, export/correction/deletion exercises, monitoring, retention, authenticated/runtime-provider evidence, and separate production approval remain open. Mailbox/contingency work is scheduled for October 7–10 and is a hard gate before the planned October 19 submission. PR #8, CI #312, the matching Ready Preview, copy/readiness checks, and anonymous smoke provide fresh remote evidence for `92b61b9` through documentation checkpoint `6d6a8aa`; the strict protected-runtime gate remains intentionally unexecuted without its protected values. A deployment approval would still not activate privacy intake, account deletion, retention, or App Store server processing.
