...
Status: **reconciled to exact committed source candidate `3dbba3a`; no production action is authorized.** The production migration-history and aggregate migration-safety preflight were repeated read-only on September 11 against the existing intended-production connection for parent `bbe63e4`; `3dbba3a` changes only dead demo helpers/imports and no migration, dependency, privacy, billing, or deletion path. The preflight confirms exactly the four expected pending migrations and the unchanged reviewed orphan/sentinel snapshot. No migration, deployment, protected configuration, account deletion, provider mutation, App Store Connect change, signed build, upload, distribution, submission, or release occurred.
...
| --- | --- |
| Current source candidate | `3dbba3a260b10924dff254deed7f65a5e392c239` on `codex/logo-refresh-sept10`, synchronized with its remote. Parent `bbe63e4` passed the isolated frozen install, production/full audits, release-source checks, TypeScript, ESLint, and Prisma generation. Exact-head CI `34666462130` exposed four unused demo paths; `3dbba3a` removes only those paths, passes strict TypeScript and release-source checks locally, and passes [exact-head CI `34668039916`](https://github.com/Tylerandersongates/Capitol-Ledger/actions/runs/34668039916). Matching Vercel deployment `6K9Xd24c4Pg4Nz1pRLorNzo6MxGb` is Ready, and anonymous Preview smoke passes on `/dashboard`, `/search?type=bills`, and `/brief`. The local Next build exhausted the constrained host's 512 MB heap; Vercel built the exact SHA successfully. `jsrsasign@11.1.5`, provider, sandbox, device, deployment, and activation gates remain open. See the [September 11 dependency note](dependency-security-audit-2026-09-11.md). |
...
Candidate `bbe63e4` changes source, dependencies, the lockfile, migrations, readiness checks, native/billing behavior, and blank-account presentation after `a6cb1da`. The earlier CI, Preview, smoke, and `f4f04de` dependency-audit closure remain valid historical evidence only. The new exact candidate has a clean isolated frozen install, no known production/full advisory findings, passing release-source/TypeScript/ESLint checks, normal Prisma generation, and matching Ready Vercel Preview. Exact-head CI and proportionate Preview smoke must still close, the local build memory limitation must be reconciled, and the unmaintained `jsrsasign@11.1.5` monitoring/upgrade and owner-acceptance gate remains separate. Local fixtures are preparation, not Apple sandbox or device proof.
...
The read-only preflight was repeated after exact candidate `bbe63e4` added `20260911110000_app_store_server_state`. Prisma found 16 repository migrations and reported exactly the four migrations in the required order below as unapplied. The aggregate-only checks repeated in a read-only serializable transaction and matched the recorded snapshot: zero Official Contact, Petition, Team-pause user, or unexpected workspace orphans; two Team-pause rows; zero known sentinels; zero null workspaces; `AccountDeletionRequest` present; `AccountDeletionCleanupJob` absent; and `TeamSubscriptionPause.workspaceId` still non-nullable. Repeat these checks immediately before any Batch A action; any changed result is a stop condition.
...
Batch B is blocked until the committed candidate's strict CI, matching Preview/smoke, and `jsrsasign` maintenance-risk gates are closed and the default-off activation sequence above is approved and evidenced. Only after Batch A passes, identify the exact production commit containing the reviewed payload and no unreviewed source or dependency changes. Require clean production/full frozen-candidate audits, fresh strict CI, a matching Preview, and release/readiness evidence; neither the `a6cb1da` artifacts nor preliminary evidence for a different source SHA applies. Deploy through the existing controlled production path while deletion and retention remain unavailable, then perform non-destructive HTTP/visual smoke on `/brief`, `/privacy`, `/support`, `/settings`, and direct `/account-deleted`; verify private account APIs reject anonymous access and the destructive action cannot be invoked before activation approval. This batch authorizes no activation, protected task configuration, provider mutation, App Store change, Notifications V2 activation, or destructive QA.
...
| Default-off deletion and retention source gates | Committed in candidate `3dbba3a` and source-verified; not fully candidate-verified, approved, deployed, configured, or activated |
