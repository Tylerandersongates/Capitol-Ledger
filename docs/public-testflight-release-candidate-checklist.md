...
Status: preparation only; the current build-upload decision remains no-go. July 29–31 evidence below is historical; current status pointers were reconciled September 10, 2026 after the approved wordmark release, recovery of the superseding Sentry privacy/native-monitoring/Apple-signing evidence, and T03's failed dependency gate. Production source `7ec68bc` is live, but no new signed/device candidate or TestFlight upload was verified.

This is the go/no-go checklist for public or external TestFlight testing. Use the [September 10 EOD](eod-handoff-2026-09-10.md), [current timeline/task ledger](project-timeline.md) and [T03 dependency audit](dependency-security-audit-2026-09-10.md) for current ordering and carryovers; the July 29–31 safety gates below remain binding. T01 is complete with the logo retained and only the shared wordmark-card `CE` suffix removed. T02 is complete for new events based on recovered commits `46efc95` and `d7980aa`. T03's fresh audit completed but failed the release gate with 2 critical, 11 high and 2 moderate production advisories; remediation plus re-audit or a new exact candidate-specific exception is required. Corrected-wordmark listing screenshot recapture remains under T09. Reserve the October 2–6 owner-availability buffer when scheduling approvals/device work. App Store Connect setup notes and earlier beta guides are reference material only. Revalidate candidate-specific checks against the exact native candidate; July checkmarks and risk acceptance are not verification or acceptance of the September findings.
...
- [x] Frozen pnpm 9.15.9 installs pass under supported Node 22 in pull-request and post-merge `main` CI. A July 29 local reinstall was stopped by a historical external minimum-package-age guard for 17 recently published lockfile entries; the guard was not bypassed. Its owner, window and enforcement source are not tracked in this checkout and must be identified before relying on it for a future reinstall.
...
- [x] The September 10 T03 audit completed with exact Node `22.22.3` and pnpm `9.15.9`, without an install or dependency change. It reports 15 production advisories—2 critical, 11 high and 2 moderate—and 18 package-version findings across the full graph (17 distinct advisory IDs). Audit evidence is complete; the release gate failed.
...
- [ ] Resolve the September 10 T03 dependency gate or obtain Tyler's new exact candidate-specific exception. The fresh production audit has 2 critical, 11 high and 2 moderate findings; both criticals affect direct `next@15.5.22` and are patched from `15.5.24`. The recommended narrow candidate is aligned Next `15.5.25`, followed by pnpm 9 lock refresh, full regression and re-audit. No dependency change or new exception is authorized yet; do not use unsafe overrides merely to silence findings.
...
| `docs/dependency-security-audit-2026-09-10.md` | Current T03 audit result, reachability analysis, failed-gate disposition and approval-gated remediation plan. |
