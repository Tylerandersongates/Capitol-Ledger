...
- [x] Temporary, secret-gated Preview diagnostics verified Node.js and Edge runtime delivery. Unauthorized requests returned 404; one fixed synthetic event from each runtime was accepted and flushed. The one-time Preview secret and all diagnostic routes were removed afterward and were never merged to `main`.
- [ ] Resolve the Preview diagnostic privacy and mapping findings before external testing: the Edge event retained request IP/geography context despite `sendDefaultPii: false`, and its top stack frame remained a generated Vercel Edge frame instead of mapping to the route source. The Node.js event mapped to route source and did not show the same stored IP context.
...
- [ ] Complete protected Sentry privacy/mapping remediation and native crash-delivery verification. Browser transport, feedback delivery, Node.js and Edge runtime ingestion, production release creation, and source-map artifact upload are verified; Edge request-IP retention and stack mapping remain blocking.
...
- [ ] Verify one non-sensitive in-app feedback report plus browser, server, edge, and native diagnostics with PII and replay safeguards intact. Production feedback, browser transport, and Preview Node.js/Edge ingestion are verified. Edge IP retention and stack mapping remain blocking, and native diagnostics remain.
