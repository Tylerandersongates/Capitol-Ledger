...
export function normalizeAccountGamification(value: Partial<AccountGamificationSnapshot> = {}): AccountGamificationSnapshot {
  ...
  return {
    ...
    earnedBadgeIds,
    eventCounts,
    level: toPositiveInteger(value.level) || summary.level,
    levelTitle: value.levelTitle || summary.levelTitle,
    monthlyGain: toPositiveInteger(value.monthlyGain) || summary.monthlyGain,
    nextLevelScore: toPositiveInteger(value.nextLevelScore) || summary.nextLevelScore,
    totalActions: summary.totalActions,
    totalBadges: summary.totalBadges,
    updatedAt: new Date().toISOString(),
    xpProgress: Math.round((civicScore / (toPositiveInteger(value.nextLevelScore) || summary.nextLevelScore)) * 100)
  };
}
