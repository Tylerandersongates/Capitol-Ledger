...
function allowLegacyDemoVerificationCode() {
  return process.env.AUTH_ALLOW_DEMO_VERIFICATION_CODE === "true" && process.env.NODE_ENV !== "production";
}
...
export async function verifyEmailToken({ code, sessionToken, token }: { code?: string; sessionToken?: string; token?: string }): Promise<EmailVerificationResult> {
  ...
  if (sessionToken && code === "1234" && allowLegacyDemoVerificationCode()) {
  ...
  if (code && !token) {
    return {
      configured: true,
      error: "Use the secure verification link from your email to verify this account.",
      status: 400
    };
  }
