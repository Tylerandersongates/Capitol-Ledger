...
assert.ok(votePage.includes("Member positions"), "Vote detail should label the member-level list neutrally");
assert.ok(!votePage.includes("Party breakdown"), "Vote detail should not lead with partisan party-breakdown framing");
...
assert.ok(memberBreakdown.includes("partyFilters"), "Vote detail member breakdown should offer party filters");
assert.ok(memberBreakdown.includes("partyFilter === \"all\" || record.member?.party === partyFilter"), "Vote detail member breakdown should filter by party only when selected");
assert.ok(!memberBreakdown.includes("groupPositionsByParty"), "Vote detail member breakdown should not auto-group all positions by party");
